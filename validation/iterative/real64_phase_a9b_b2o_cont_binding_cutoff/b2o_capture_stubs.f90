module B2O_STUB_PROBES
  use, intrinsic :: iso_fortran_env, only : int64, real64
  implicit none
  integer, parameter :: B2O_NUNKNO=14, B2O_NGRP=370
  integer(int64), parameter :: B2O_CUTOFF_SENTINEL = &
      4294967311_int64
  integer, save :: xdrta2_calls=0, core_calls=0
  integer, save :: xdrta2_total=0, core_total=0
  logical, save :: capture_valid=.false.
  real(real64), save :: captured_flux64(B2O_NUNKNO,B2O_NGRP)=0.0_real64
  real(real64), save :: captured_qfiss64(B2O_NUNKNO,B2O_NGRP)=0.0_real64
contains
  subroutine B2O_RESET_TOTALS()
    xdrta2_calls=0
    core_calls=0
    xdrta2_total=0
    core_total=0
  ...
  end subroutine B2O_RESET_TOTALS

  subroutine B2O_RESET_PROBES()
    xdrta2_calls=0
    core_calls=0
    capture_valid=.false.
    captured_flux64=0.0_real64
    captured_qfiss64=0.0_real64
  end subroutine B2O_RESET_PROBES
end module B2O_STUB_PROBES
...
module SPOR64_A9
  ...
  use B2O_STUB_PROBES, only : B2O_CUTOFF_SENTINEL, core_calls, &
      core_total, capture_valid, captured_flux64, captured_qfiss64
  implicit none
  private
...
contains
  subroutine FLU2DR64_CORE(jpsys_group,iptrk,iftrak,impx,title, &
    ...
    cutoff_visit64=B2O_CUTOFF_SENTINEL
    accepted=.true.
    ok=.true.
...
subroutine XDRTA2()
  use B2O_STUB_PROBES, only : xdrta2_calls, xdrta2_total
  implicit none
