...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_current_ptu_aa1_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-current-ptu-aa1-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_current_ptu_aa1_candidate.sh"
EXPECTED_AX_SHA=75dd254844b001596f3e57e6aad373e6bdec7aa84958e79b9111c555e93556cf
EXPECTED_SNAP_SHA=169be7e8f17e7b1985b65ae2522966bbba0768febadb2a031f0e0a860e0a11fd
...
test "$(awk 'NF && $1 !~ /^#/ && NF != 3 {n++} END {print n+0}' \
  "$MANIFEST")" = 0
for role in p t u u_snapshots basis_reference
do
  ...
  case "$relative" in
    /*|.|..|../*|*/../*|*/..)
      printf 'RANK2-CURRENT-PTU-AA1 ERROR: unsafe path for %s\n' "$role" >&2
      exit 2
      ;;
  esac
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-current-ptu-aa1.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-current-ptu-aa1.$$"
cleanup() {
  rm -rf "$WORK"
...
for binding in p:p t:t u:u us:u_snapshots basis:basis_reference
do
  short=${binding%%:*}
  role=${binding#*:}
...
then
  printf '%s\n' 'RANK2-CURRENT-PTU-AA1 ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate --consecutive-ptu-screened \
    p.xsm t.xsm u.xsm us.xsm \
    proposal_axial.xsm proposal_snapshots.xsm >build.log
  ./check_candidate --consecutive-ptu-screened \
    p.xsm t.xsm u.xsm us.xsm basis.xsm \
    proposal_axial.xsm proposal_snapshots.xsm >check.log
)
rg -q '^RANK2-CURRENT-PTU-AA1 PARAMETER-FREE DIRECTION GATE PASS$' \
  "$WORK/build.log" "$WORK/check.log"
rg -q '^RANK2-CURRENT-PTU-AA1 PROPOSAL COMPLETE NOT-EVALUATED NO-DRAGON NO-MAP$' \
  "$WORK/build.log"
rg -q '^RANK2-CURRENT-PTU-AA1 COMPLETE$' "$WORK/check.log"
...
cp "$RUNNER" "$STAGE/run_rank2_current_ptu_aa1_candidate.sh"
...
(
  ...
  shasum -a 256 proposal_axial.xsm proposal_snapshots.xsm build.log \
    ...
    run_rank2_current_ptu_aa1_candidate.sh classification.txt \
    >result.sha256
...
printf '%s\n' \
  "RANK2-CURRENT-PTU-AA1 AX-SHA256=$ax_sha" \
  "RANK2-CURRENT-PTU-AA1 SNAP-SHA256=$snap_sha" \
  'RANK2-CURRENT-PTU-AA1 INPUTS-READ-ONLY HASH PASS' \
  'RANK2-CURRENT-PTU-AA1 RECEIPT 9/9 PASS' \
  'RANK2-CURRENT-PTU-AA1 DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-CURRENT-PTU-AA1 CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-CURRENT-PTU-AA1 RESULT=$ARTIFACT_DIR"
