...
case "$RUN_RANK2_CURRENT_CEFG_AA2_MAP" in
  0)
    echo "SPOT-RANK2-CURRENT-CEFG-AA2-MAP DEFAULT-OFF: no Dragon process started."
    exit 0
    ;;
  1) ;;
  *)
    echo "SPOT-RANK2-CURRENT-CEFG-AA2-MAP ERROR: activation must be 0 or 1." >&2
    exit 2
    ;;
esac

ROOT=$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd)
PROPOSAL_DIR="$ROOT/validation/artifacts/iterative-rank2-current-cefg-aa2-candidate"
...
RUN_CONTINUATION=1 \
RESULT_DIR="$ROOT/validation/artifacts/iterative-rank2-current-cefg-aa2-map" \
PARENT_MANIFEST="$ROOT/validation/iterative/rank2_current_cefg_aa2_map_parent.tsv" \
CONTINUATION_POLICY_SOURCE="$ROOT/validation/iterative/rank2_current_cefg_aa2_map_policy.md" \
