...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_modal_aa1_next_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-modal-aa1-next-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_modal_aa1_next_candidate.sh"
EXPECTED_AX_SHA=c2df5e526aa9a0c0c3dc354d3ec539475814fe73c87af19ba04dde05c1475ff4
EXPECTED_SNAP_SHA=530d23485baa006342b08815a9240d61fa4ec63810103307ead66104eb5bef9c
...
test "$bad_width" = 0
for role in x1 x2 y_pub z z_snapshots basis_reference
do
  ...
  case "$relative" in
    /*|.|..|../*|*/../*|*/..)
      printf 'RANK2-MODAL-AA1-NEXT ERROR: unsafe path for %s\n' \
        "$role" >&2
      exit 2
      ;;
  esac
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-modal-aa1-next.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-modal-aa1-next-candidate.$$"
cleanup() {
  rm -rf "$WORK"
...
then
  printf '%s\n' \
    'RANK2-MODAL-AA1-NEXT ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate --next x1.xsm x2.xsm y_pub.xsm z.xsm \
    z_snapshots.xsm proposal_axial.xsm proposal_snapshots.xsm > build.log
  ./check_candidate --next x1.xsm x2.xsm y_pub.xsm z.xsm \
...
cp "$RUNNER" "$STAGE/run_rank2_modal_aa1_next_candidate.sh"
...
(
  ...
  shasum -a 256 \
    ...
    run_rank2_modal_aa1_next_candidate.sh classification.txt \
    > result.sha256
...
printf '%s\n' \
  "RANK2-MODAL-AA1-NEXT AX-SHA256=$ax_sha" \
  "RANK2-MODAL-AA1-NEXT SNAP-SHA256=$snap_sha" \
  'RANK2-MODAL-AA1-NEXT INPUTS-READ-ONLY HASH PASS' \
  'RANK2-MODAL-AA1-NEXT DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-MODAL-AA1-NEXT CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-MODAL-AA1-NEXT RESULT=$ARTIFACT_DIR"
