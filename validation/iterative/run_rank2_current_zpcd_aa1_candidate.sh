...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_current_zpcd_aa1_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-current-zpcd-aa1-candidate"}
...
test "$(awk 'NF && $1 !~ /^#/ && NF != 3 {n++} END {print n+0}' \
  "$MANIFEST")" = 0
for role in z z_plus c_aa1_pub d d_snapshots basis_reference
do
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-zpcd-aa1.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-zpcd-aa1.$$"
cleanup() {
  rm -rf "$WORK"
...
for binding in z:z zp:z_plus c:c_aa1_pub d:d dsnap:d_snapshots basis:basis_reference
do
  short=${binding%%:*}
  role=${binding#*:}
...
then
  printf '%s\n' 'RANK2-CURRENT-ZPCD-AA1 ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate --next-zpcd-screened z.xsm zp.xsm c.xsm d.xsm \
    dsnap.xsm proposal_axial.xsm proposal_snapshots.xsm >build.log
  ./check_candidate --next-zpcd-screened z.xsm zp.xsm c.xsm d.xsm \
    dsnap.xsm basis.xsm proposal_axial.xsm \
    proposal_snapshots.xsm >check.log
)
rg -q '^RANK2-CURRENT-ZPCD-AA1 PARAMETER-FREE DIRECTION GATE PASS$' \
  "$WORK/build.log" "$WORK/check.log"
rg -q '^RANK2-CURRENT-ZPCD-AA1 CARRIER AA1-RAW-FLUX$' "$WORK/build.log"
rg -q '^RANK2-CURRENT-ZPCD-AA1 COMPLETE$' "$WORK/check.log"
...
cp "$RUNNER" "$STAGE/run_rank2_current_zpcd_aa1_candidate.sh"
...
(
  ...
  shasum -a 256 proposal_axial.xsm proposal_snapshots.xsm build.log \
    ...
    run_rank2_current_zpcd_aa1_candidate.sh classification.txt \
    >result.sha256
...
printf '%s\n' \
  "RANK2-CURRENT-ZPCD-AA1 AX-SHA256=$ax_sha" \
  "RANK2-CURRENT-ZPCD-AA1 SNAP-SHA256=$snap_sha" \
  'RANK2-CURRENT-ZPCD-AA1 INPUTS-READ-ONLY HASH PASS' \
  'RANK2-CURRENT-ZPCD-AA1 RECEIPT 9/9 PASS' \
  'RANK2-CURRENT-ZPCD-AA1 DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-CURRENT-ZPCD-AA1 CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-CURRENT-ZPCD-AA1 RESULT=$ARTIFACT_DIR"
