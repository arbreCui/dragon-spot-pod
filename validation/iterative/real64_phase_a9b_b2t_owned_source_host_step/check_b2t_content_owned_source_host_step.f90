program CHECK_B2T_CONTENT_OWNED_SOURCE_HOST_STEP
  use GANLIB
  ...
  integer :: argument, other, plane
  integer :: mat(NREG), keyflx(NREG), epoch
  integer :: phi_count, qfiss_count, dsour_count, qint_count
  integer :: nusigf_count, nonregion_count
  integer :: total_phi, total_qfiss, total_dsour, total_qint
  integer :: total_nusigf, total_nonregion
  integer(int64) :: records, words, total_records, total_words
  ...
  type(c_ptr) :: projected, macros(NSNAP), sources(NSNAP), input_macro
  ...
  do argument=1,7
    ...
    if (len_trim(paths(argument)) == 0) &
      error stop 'empty B2T content posterior path is forbidden'
  end do
  do argument=1,6
    do other=argument+1,7
      if (trim(paths(argument)) == trim(paths(other))) &
        error stop 'B2T content posterior paths must be distinct'
    end do
  end do
  ...
  do plane=1,NSNAP
    call LCMOP(macros(plane),trim(paths(2*plane)),2,2,0)
    call LCMOP(sources(plane),trim(paths(2*plane+1)),2,2,0)
    call REQUIRE_ASSOCIATED(macros(plane),'persistent MACRO evidence')
    call REQUIRE_ASSOCIATED(sources(plane),'persistent SOURCE evidence')
  end do
  do plane=1,NSNAP
    if (c_associated(projected,macros(plane)) .or. &
        c_associated(projected,sources(plane)) .or. &
        c_associated(macros(plane),sources(plane))) &
      error stop 'B2T content posterior handles alias'
    do other=plane+1,NSNAP
      if (c_associated(macros(plane),macros(other)) .or. &
          c_associated(macros(plane),sources(other)) .or. &
          c_associated(sources(plane),macros(other)) .or. &
          c_associated(sources(plane),sources(other))) &
        error stop 'B2T content cross-plane evidence handles alias'
    end do
  end do
  ...
  total_nusigf=0
  total_nonregion=0
  total_records=0_int64
  total_words=0_int64
  do plane=1,NSNAP
    call VERIFY_PROJECTED_PLANE(projected,plane,input_macro,rho64, &
        keff64,epoch,mat,keyflx,volume32,phi64,phi_count)
    call VERIFY_MACRO_OUTPUT(input_macro,macros(plane),keff64, &
        nusigf_count,records,words)
    call RECOMPUTE_QFISS(input_macro,rho64,mat,keyflx,volume32,phi64, &
        qfiss64,qint64,nonregion_count)
    call VERIFY_SOURCE_OUTPUT(sources(plane),plane,rho64,keff64,epoch, &
        qfiss64,qint64,qfiss_count,dsour_count,qint_count)
    total_phi=total_phi+phi_count
    total_qfiss=total_qfiss+qfiss_count
    total_dsour=total_dsour+dsour_count
    total_qint=total_qint+qint_count
    total_nusigf=total_nusigf+nusigf_count
    total_nonregion=total_nonregion+nonregion_count
    total_records=total_records+records
    total_words=total_words+words
  end do

  do plane=1,NSNAP
    call LCMCL(sources(plane),1)
    call LCMCL(macros(plane),1)
  end do
  call LCMCL(projected,1)

  if (total_phi /= NSNAP*NREG*NGRP) &
    error stop 'three-plane positive region-flux inventory differs'
  if (total_qfiss /= NSNAP*NUNKNO*NGRP) &
    error stop 'three-plane REAL64 QFISS bit inventory differs'
  if (total_dsour /= NSNAP*NUNKNO*NGRP) &
    error stop 'three-plane REAL32 DSOUR bit inventory differs'
  if (total_qint /= NSNAP*NGRP) &
    error stop 'three-plane REAL32 QINT bit inventory differs'
  if (total_nusigf /= NSNAP*NGRP*NMAT*NIFIS) &
    error stop 'three-plane positive-zero NUSIGF inventory differs'
  if (total_nonregion /= NSNAP*(NUNKNO-NREG)*NGRP) &
    error stop 'three-plane non-region QFISS inventory differs'
  if (total_records <= 0_int64 .or. total_words <= 0_int64) &
    error stop 'three-plane recursive unchanged inventory is empty'

  write(*,'(A)') 'B2T CONTENT THREE-PLANE POSTERIOR PASS'
  write(*,'(A)') 'B2T CONTENT NUSIGF-POSITIVE-ZERO=284160'
  write(*,'(A)') &
      'B2T CONTENT QFISS-R64-BITS=15540 DSOUR-R32-PROJECTIONS=15540 QINT-R32-PROJECTIONS=1110'
  write(*,'(A)') &
      'B2T CONTENT MACRO-OTHER-RECORDS=RECURSIVE-BIT-IDENTICAL'
  write(*,'(A)') &
      'B2T CONTENT PLANES=1,2,3 SAME-PROJECTED-PARENT=YES'
  write(*,'(A)') &
      'B2T CONTENT DRAGON=0 ASM=0 FLU=0 TRANSPORT=0 PICARD=0'

contains

  subroutine VERIFY_PROJECTED_PLANE(root,plane_index,macro,rho,keff, &
      plane_epoch,material,key,volume,flux,positive_count)
    ...
    integer :: plane_key(NREG), anis_key(NREG), imerge(NMAT)
    ...
    if (plane_index < 1 .or. plane_index > NSNAP) &
      error stop 'B2T content plane index is invalid'
    ...
    track=LCMGIL(tracks,plane_index)
    library=LCMGIL(libraries,plane_index)
    plane=LCMGIL(planes,plane_index)
    call REQUIRE_ASSOCIATED(track,'selected PROJECTED TRACK')
    call REQUIRE_ASSOCIATED(library,'selected PROJECTED MICROLIB2')
    call REQUIRE_ASSOCIATED(plane,'selected PROJECTED FLUX')
    ...
    if (any(track_state /= TRACK_STATE_EXPECTED)) &
      error stop 'selected TRACK state differs'
    ...
    call LCMGET(track,'KEYFLX$ANIS',anis_key)
    if (any(key /= anis_key)) error stop 'selected TRACK keys differ'
    if (any(material < 1) .or. any(material > NMAT)) &
      error stop 'selected material map is invalid'
    if (.not. all(ieee_is_finite(volume)) .or. &
        any(volume <= +0.0_real32)) &
      error stop 'selected region volume is invalid'
    seen=.false.
    do ir=1,NREG
      if (key(ir) < 1 .or. key(ir) > NUNKNO) &
        error stop 'selected region key is out of range'
      if (seen(key(ir))) error stop 'selected region key is repeated'
      seen(key(ir))=.true.
    end do
    ...
    if (library_state(1) /= NMAT .or. library_state(2) <= 0 .or. &
        library_state(3) /= NGRP .or. library_state(4) /= NSNAP) &
      error stop 'selected library state differs'
    call REQUIRE_RECORD(library,'MACROLIB',-1,0)
    macro=LCMGID(library,'MACROLIB')
    call REQUIRE_ASSOCIATED(macro,'selected physical MACROLIB')
    ...
    if (any(macro_state /= MACRO_STATE_EXPECTED)) &
      error stop 'selected MACROLIB state differs'
    call REQUIRE_ABSENT(macro,'SPOT-FROZEN')
    call REQUIRE_ABSENT(macro,'SPOT-KEFF')
    call REQUIRE_RECORD(macro,'GROUP',NGRP,10)
    macro_groups=LCMGID(macro,'GROUP')
    call REQUIRE_ASSOCIATED(macro_groups,'selected MACROLIB GROUP list')
    do ig=1,NGRP
      call REQUIRE_DIRECTORY_ITEM(macro_groups,ig)
    end do

    if (.not. EXACT_INVENTORY(plane,PLANE_ROOT_NAMES)) &
      error stop 'selected PROJECTED plane inventory differs'
    ...
    if (.not. all(ieee_is_finite(leakage))) &
      error stop 'selected PROJECTED leakage is nonfinite'

    call REQUIRE_RECORD(plane,'SPOT-R64',-1,0)
    plane_authority=LCMGID(plane,'SPOT-R64')
    call REQUIRE_ASSOCIATED(plane_authority,'selected plane authority')
    if (.not. EXACT_INVENTORY(plane_authority,PLANE_AUTHORITY_NAMES)) &
      error stop 'selected plane authority inventory differs'
    ...
    if (.not. ieee_is_finite(plane_rho)) &
      error stop 'selected PROJECTED RHO is nonfinite'
    if (transfer(plane_rho,0_int64) /= transfer(root_rho,0_int64)) &
      error stop 'selected PROJECTED RHO differs from root'
    if (plane_epoch /= root_epoch) &
      error stop 'selected PROJECTED epoch differs from root'
    rho=plane_rho
    authority_flux=LCMGID(plane_authority,'FLUX')
    mirror_flux=LCMGID(plane,'FLUX')
    call REQUIRE_ASSOCIATED(authority_flux,'selected REAL64 FLUX list')
    call REQUIRE_ASSOCIATED(mirror_flux,'selected REAL32 FLUX list')
    positive_count=0
    do ig=1,NGRP
      ...
      if (.not. all(ieee_is_finite(flux(:,ig))) .or. &
        ...
        error stop 'selected PROJECTED flux is nonfinite'
      if (any(abs(flux(:,ig)) > REAL32_MAX64)) &
        error stop 'selected PROJECTED flux exceeds REAL32 range'
      if (any(transfer(mirror,0_int32,NUNKNO) /= &
          transfer(real(flux(:,ig),real32),0_int32,NUNKNO))) &
        error stop 'selected PROJECTED compatibility flux differs'
      do ir=1,NREG
        if (flux(key(ir),ig) <= +0.0_real64) &
          error stop 'selected PROJECTED region flux is not positive'
  ...
  subroutine VERIFY_MACRO_OUTPUT(input,output,expected_keff,zero_count, &
    ...
    integer :: input_count, output_count, length, record_type, marker
    ...
    if (.not. ieee_is_finite(found_keff) .or. &
        transfer(found_keff,0_int32) /= transfer(expected_keff32,0_int32)) &
      ...
      do
        if (name == 'NUSIGF') then
          ...
          if (any(transfer(output_nusigf,0_int32,NMAT*NIFIS) /= &
              0_int32)) &
            error stop 'output NUSIGF is not positive zero'
  ...
  subroutine RECOMPUTE_QFISS(macro,rho,material,key,volume,flux,qfiss, &
    ...
    do g=1,NGRP
      do ir=1,NREG
        ...
        if (.not. ieee_is_finite(qint(g)) .or. qint(g) < +0.0_real64) &
  ...
  subroutine VERIFY_SOURCE_OUTPUT(source,expected_plane,rho,keff, &
      expected_epoch,qfiss,qint,qfiss_count,dsour_count,qint_count)
    type(c_ptr), intent(in) :: source
    integer, intent(in) :: expected_plane, expected_epoch
    ...
    if (.not. ieee_is_finite(source_keff) .or. &
        transfer(source_keff,0_int32) /= transfer(expected_keff32,0_int32)) &
    ...
    if (plane /= expected_plane) error stop 'SOURCE authority plane differs'
...
end program CHECK_B2T_CONTENT_OWNED_SOURCE_HOST_STEP
