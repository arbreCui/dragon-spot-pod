...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_qsvw_aa1_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-qsvw-aa1-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_qsvw_aa1_candidate.sh"
EXPECTED_AX_SHA=18860e284a02f104821a6eba26ea7743863ccf1193e01089389685648ef7f6c0
EXPECTED_SNAP_SHA=77a3f082955c42ecdcae925a8b52f40217e129715123eca4c5fb45ce6c9440e8
...
test "$(awk 'NF && $1 !~ /^#/ && NF != 3 {n++} END {print n + 0}' \
  "$MANIFEST")" = 0
for role in qs_pub v w w_snapshots basis_reference
do
  ...
  case "$relative" in
    /*|.|..|../*|*/../*|*/..)
      printf 'RANK2-QSVW-AA1 ERROR: unsafe path for %s\n' "$role" >&2
      exit 2
      ;;
  esac
  source_file="$ROOT/$relative"
  test -f "$source_file" && test ! -L "$source_file"
  test "$(hash_file "$source_file")" = "$expected"
done

WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-qsvw-aa1.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-qsvw-aa1.$$"
cleanup() {
  rm -rf "$WORK"
...
for binding in qs:qs_pub v:v w:w ws:w_snapshots basis:basis_reference
do
  short=${binding%%:*}
  role=${binding#*:}
...
then
  printf '%s\n' 'RANK2-QSVW-AA1 ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate --consecutive-current qs.xsm v.xsm w.xsm ws.xsm \
    proposal_axial.xsm proposal_snapshots.xsm >build.log
  ./check_candidate --consecutive-current qs.xsm v.xsm w.xsm ws.xsm \
    basis.xsm proposal_axial.xsm proposal_snapshots.xsm >check.log
)
rg -q '^RANK2-QSVW-AA1 PROPOSAL COMPLETE NOT-EVALUATED NO-DRAGON NO-MAP$' \
  "$WORK/build.log"
rg -q '^RANK2-QSVW-AA1 COMPLETE$' "$WORK/check.log"
rg -q '^RANK2-QSVW-AA1 LEAKAGE SCREEN ONLY NO LEAKAGE FIT$' \
...
cp "$RUNNER" "$STAGE/run_rank2_qsvw_aa1_candidate.sh"
...
(
  ...
  shasum -a 256 \
    ...
    run_rank2_qsvw_aa1_candidate.sh classification.txt >result.sha256
...
printf '%s\n' \
  "RANK2-QSVW-AA1 AX-SHA256=$ax_sha" \
  "RANK2-QSVW-AA1 SNAP-SHA256=$snap_sha" \
  'RANK2-QSVW-AA1 INPUTS-READ-ONLY HASH PASS' \
  'RANK2-QSVW-AA1 DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-QSVW-AA1 CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-QSVW-AA1 RESULT=$ARTIFACT_DIR"
