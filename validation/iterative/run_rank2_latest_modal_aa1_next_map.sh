...
RUN_RANK2_LATEST_MODAL_AA1_NEXT_MAP=${RUN_RANK2_LATEST_MODAL_AA1_NEXT_MAP:-0}
case "$RUN_RANK2_LATEST_MODAL_AA1_NEXT_MAP" in
  0)
    echo "SPOT-RANK2-LATEST-MODAL-AA1-NEXT-MAP DEFAULT-OFF: no Dragon process started."
    exit 0
    ;;
  1) ;;
  *)
    echo "SPOT-RANK2-LATEST-MODAL-AA1-NEXT-MAP ERROR: activation must be 0 or 1." >&2
    exit 2
    ;;
esac
...
RUN_CONTINUATION=1 \
RESULT_DIR="$ROOT/validation/artifacts/iterative-rank2-latest-modal-aa1-next-map" \
PARENT_MANIFEST="$ROOT/validation/iterative/rank2_latest_modal_aa1_next_map_parent.tsv" \
CONTINUATION_POLICY_SOURCE="$ROOT/validation/iterative/rank2_latest_modal_aa1_next_map_policy.md" \
