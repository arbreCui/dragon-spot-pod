...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_modal_aa2_rolling_next_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-modal-aa2-rolling-next-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_modal_aa2_rolling_next_candidate.sh"
EXPECTED_AX_SHA=ce9544e8f58b01d933f5937d781a0f1b70a58862468bbd380d5d2bc5c0e07715
EXPECTED_SNAP_SHA=61604c1dfb586abe71115544aa73b4628f7528f5da32f6bce14ce8f7f8f30763
...
test "$bad_width" = 0
for role in x0_pub x0_plus x1_pub x1_plus x2_pub x2_plus \
...
do
  ...
  case "$relative" in
    /*|.|..|../*|*/../*|*/..)
      printf 'RANK2-ROLLING-AA2-NEXT ERROR: unsafe path for %s\n' \
        "$role" >&2
      exit 2
      ;;
  esac
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-rolling-aa2-next.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-modal-aa2-rolling-next-candidate.$$"
cleanup() {
  rm -rf "$WORK"
...
for binding in x0:x0_pub x0p:x0_plus x1:x1_pub x1p:x1_plus \
  x2:x2_pub x2p:x2_plus x2ps:x2_plus_snapshots basis:basis_reference
do
  short=${binding%%:*}
  role=${binding#*:}
...
then
  printf '%s\n' \
    'RANK2-ROLLING-AA2-NEXT ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate --rolling-aa2-next \
    x0.xsm x0p.xsm x1.xsm x1p.xsm x2.xsm x2p.xsm x2ps.xsm \
    proposal_axial.xsm proposal_snapshots.xsm > build.log
  ./check_candidate --rolling-aa2-next \
    x0.xsm x0p.xsm x1.xsm x1p.xsm x2.xsm x2p.xsm x2ps.xsm basis.xsm \
    proposal_axial.xsm proposal_snapshots.xsm > check.log
)
rg -q '^RANK2-ROLLING-AA2-NEXT CARRIER AA2-RAW-FLUX$' "$WORK/build.log"
rg -q '^RANK2-ROLLING-AA2-NEXT COMPLETE$' "$WORK/check.log"
...
cp "$RUNNER" "$STAGE/run_rank2_modal_aa2_rolling_next_candidate.sh"
...
(
  ...
  shasum -a 256 \
    ...
    run_rank2_modal_aa2_rolling_next_candidate.sh classification.txt \
    > result.sha256
...
printf '%s\n' \
  "RANK2-ROLLING-AA2-NEXT AX-SHA256=$ax_sha" \
  "RANK2-ROLLING-AA2-NEXT SNAP-SHA256=$snap_sha" \
  'RANK2-ROLLING-AA2-NEXT INPUTS-READ-ONLY HASH PASS' \
  'RANK2-ROLLING-AA2-NEXT DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-ROLLING-AA2-NEXT CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-ROLLING-AA2-NEXT RESULT=$ARTIFACT_DIR"
