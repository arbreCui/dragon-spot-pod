program build_rank2_basis_xsm
  ! Rebuild rank-one and rank-two POD records from the same frozen raw
  ...
  ! rank-one reference prepared by the runner.
  use GANLIB
  ...
  real(real32) :: packed1(1,nsnap),packed2(2,nsnap)
  real(real32) :: ref_volume(nreg),ref_basis(nreg),ref_coeff(nsnap)
  ...
  if (any(rank_root /= 1)) call fail('REFERENCE IS NOT RANK ONE.')
  ...
  do g=1,ngroup
    ...
    call SPOPOD(nreg,nsnap,phi,volume(:,1),1,nmode1,basis1,coeff1, &
      sigma1,rec1,ortho1,0,g)
    call SPOPOD(nreg,nsnap,phi,volume(:,1),2,nmode2,basis2,coeff2, &
      sigma2,rec2,ortho2,0,g)
    if ((nmode1 /= 1).or.(nmode2 /= 2)) &
    ...
    if (any(real32_bits(basis1(:,1)) /= real32_bits(basis2(:,1))).or. &
        any(real32_bits(coeff1(1,:)) /= real32_bits(coeff2(1,:)))) &
      call fail('RANK-TWO PREFIX DIFFERS FROM FRESH RANK ONE.')
    ...
    if (any(real32_bits(ref_basis) /= real32_bits(basis1(:,1))).or. &
        any(real32_bits(ref_coeff) /= real32_bits(coeff1(1,:))).or. &
    ...
    call store_group(control_group,1,volume(:,1),basis1(:,:1),packed1, &
      sigma1,rec1,ortho1)
    call store_group(candidate_group,2,volume(:,1),basis2(:,:2),packed2, &
      sigma2,rec2,ortho2)
  ...
  write(6,'(A,I0,A,I0,A,I0)') 'RANK2-BUILDER GROUPS=',ngroup, &
    ' REGIONS=',nreg,' SNAPSHOTS=',nsnap
  write(6,'(A)') 'RANK2-BUILDER FRESH-RANK1 BITWISE PASS'
  write(6,'(A)') 'RANK2-BUILDER PREFIX BITWISE PASS'
  write(6,'(A)') 'RANK2-BUILDER COMPLETE'
...
contains
  ...
  subroutine load_reference_group(ptr,group,vol,basis,coeff,sigma,rec,ortho)
    ...
    real(real32), intent(out) :: vol(nreg),basis(nreg),coeff(nsnap)
    ...
    if ((nreg0 /= nreg).or.(nsnap0 /= nsnap).or.(nmode0 /= 1)) &
      call fail(trim(label)//' INVALID DIMENSIONS.')
    call require_record(ptr,'VOL2D',nreg,2,label)
    call require_record(ptr,'POD-BASIS',nreg,2,label)
    call require_record(ptr,'POD-COEFF',nsnap,2,label)
...
end program build_rank2_basis_xsm
