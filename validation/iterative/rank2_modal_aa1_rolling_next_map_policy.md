...
\[
G_2(x_{\mathrm{roll2}}),\qquad
x_{\mathrm{roll2}}=
0.79412955189905154x_{\mathrm{next}}^+
+0.20587044810094848x_{\mathrm{roll}}^+.
\]

The hash-locked parent is the independently materialized standard AA(1)
proposal. Its canonical `(A,rho,L)` fields contain the unmodified affine
state. `XRP-RAW-FLUX` identifies the complete latest-returned
\(x_{\mathrm{roll}}^+\) AX and snapshot carrier; it does not replace the
...
`PROPOSAL + XRP-RAW-FLUX` Ganlib preflight must pass before either Dragon
invocation. One activation permits one attempt and no retry, relaxation,
damping, clipping, fitted closure, regularization or empirical coefficient.
...
adequacy or physical accuracy, and it starts no successor proposal, AA(2) or
map. Preparing this host performs no Dragon, assembly, transport or Picard
calculation and produces no map result.

## Post-run record

The frozen host was subsequently activated exactly once from source commit
`0f31132a29e4fd5266abfbe5d9647920af16f637`. All strict terminals, the
...
retry, successor proposal, AA(2) stage or successor map was started. See
