#!/usr/bin/env python3
"""Run one B2v child in a fresh process group with frozen resource caps."""
...
def fail(message: str) -> None:
    raise SystemExit(f"B2V BOUNDED PROCESS FAILURE: {message}")
...
def run(
    ...
    if return_code < 0:
        ...
        if signal_number in invalid_signals:
            fail(
                f"terminated by managed/resource signal {signal_number}; "
                f"{RESOURCE_CLASS[profile_name]}"
            )
        fail(
            f"terminated by failure signal {signal_number}; "
            f"{EXIT_CLASS[profile_name]}"
        )
    if return_code != 0:
...
def main() -> None:
    ...
    print(
        f"B2V BOUNDED PROCESS PASS PROFILE={profile_name} "
