...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_latest_modal_aa1_recovery_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-latest-modal-aa1-recovery-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_latest_modal_aa1_recovery_candidate.sh"
EXPECTED_AX_SHA=53f6bb3e48ef583778e54ce0e21ff68f5f63d3d3857c3211c0803bc9a2ef0193
EXPECTED_SNAP_SHA=3404d4295b8f71fa20d9b565fc88c0631184775dcac336be2f50e797999cefaa
...
test "$bad_width" = 0
for role in qy_pub z qt_pub u u_snapshots basis_reference
do
  ...
  case "$relative" in
    /*|.|..|../*|*/../*|*/..)
      printf 'RANK2-LATEST2-AA1-NEXT ERROR: unsafe path for %s\n' \
        "$role" >&2
      exit 2
      ;;
  esac
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-latest2-aa1-next.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-latest-modal-aa1-recovery-candidate.$$"
cleanup() {
  rm -rf "$WORK"
...
then
  printf '%s\n' \
    'RANK2-LATEST2-AA1-NEXT ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate --next-x4z qy_pub.xsm z.xsm qt_pub.xsm u.xsm \
    u_snapshots.xsm proposal_axial.xsm proposal_snapshots.xsm > build.log
  ./check_candidate --next-x4z qy_pub.xsm z.xsm qt_pub.xsm u.xsm \
    u_snapshots.xsm basis_reference.xsm proposal_axial.xsm \
...
cp "$RUNNER" "$STAGE/run_rank2_latest_modal_aa1_recovery_candidate.sh"
...
(
  ...
  shasum -a 256 \
    ...
    run_rank2_latest_modal_aa1_recovery_candidate.sh classification.txt \
    > result.sha256
...
printf '%s\n' \
  "RANK2-LATEST2-AA1-NEXT AX-SHA256=$ax_sha" \
  "RANK2-LATEST2-AA1-NEXT SNAP-SHA256=$snap_sha" \
  'RANK2-LATEST2-AA1-NEXT INPUTS-READ-ONLY HASH PASS' \
  'RANK2-LATEST2-AA1-NEXT DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-LATEST2-AA1-NEXT CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-LATEST2-AA1-NEXT RESULT=$ARTIFACT_DIR"
