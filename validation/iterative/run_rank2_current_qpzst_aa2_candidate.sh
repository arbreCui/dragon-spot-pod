...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_current_qpzst_aa2_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-current-qpzst-aa2-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_current_qpzst_aa2_candidate.sh"
EXPECTED_AX_SHA=6c72a3d812d88c33af9690629f60a964cffd42ea51326644cf36918508be0987
EXPECTED_SNAP_SHA=c6603a53e4a0d4dd46b67eca7f9b72939187970ef157a3354fcbb2e354f7431f
...
test "$(sed -n '1p' "$MANIFEST")" = \
  '# spot-rank2-current-qpzst-aa2-candidate-inputs-v1'
test "$(awk 'NF && $1 !~ /^#/ {n++} END {print n+0}' "$MANIFEST")" = 7
test "$(awk 'NF && $1 !~ /^#/ && NF != 3 {n++} END {print n+0}' \
  "$MANIFEST")" = 0
for role in q_aa2_pub p z s_z_pub t t_snapshots basis_reference
do
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-current-qpzst-aa2.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-current-qpzst-aa2.$$"
cleanup() {
  rm -rf "$WORK"
...
for binding in q:q_aa2_pub p:p z:z s:s_z_pub t:t ts:t_snapshots \
...
then
  printf '%s\n' 'RANK2-CURRENT-QPZST-AA2 ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate --current-qpzst-aa2 q.xsm p.xsm p.xsm z.xsm \
    s.xsm t.xsm ts.xsm proposal_axial.xsm proposal_snapshots.xsm \
    >build.log
  ./check_candidate --current-qpzst-aa2 q.xsm p.xsm p.xsm z.xsm \
    s.xsm t.xsm ts.xsm basis.xsm proposal_axial.xsm \
    proposal_snapshots.xsm >check.log
)
rg -q '^RANK2-CURRENT-QPZST-AA2 PARAMETER-FREE DIRECTION GATE PASS$' \
  "$WORK/build.log" "$WORK/check.log"
rg -q '^RANK2-CURRENT-QPZST-AA2 CARRIER AA2-RAW-FLUX$' "$WORK/build.log"
rg -q '^RANK2-CURRENT-QPZST-AA2 COMPLETE$' "$WORK/check.log"
...
cp "$RUNNER" "$STAGE/run_rank2_current_qpzst_aa2_candidate.sh"
...
(
  ...
  shasum -a 256 proposal_axial.xsm proposal_snapshots.xsm build.log \
    ...
    run_rank2_current_qpzst_aa2_candidate.sh classification.txt \
    >result.sha256
...
printf '%s\n' \
  "RANK2-CURRENT-QPZST-AA2 AX-SHA256=$ax_sha" \
  "RANK2-CURRENT-QPZST-AA2 SNAP-SHA256=$snap_sha" \
  'RANK2-CURRENT-QPZST-AA2 INPUTS-READ-ONLY HASH PASS' \
  'RANK2-CURRENT-QPZST-AA2 RECEIPT 9/9 PASS' \
  'RANK2-CURRENT-QPZST-AA2 DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-CURRENT-QPZST-AA2 CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-CURRENT-QPZST-AA2 RESULT=$ARTIFACT_DIR"
