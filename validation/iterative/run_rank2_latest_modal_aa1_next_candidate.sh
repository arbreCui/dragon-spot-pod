...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_latest_modal_aa1_next_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-latest-modal-aa1-next-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_latest_modal_aa1_next_candidate.sh"
EXPECTED_AX_SHA=e9e37246df25ef9afb449fad77e55b6ce21cd03f09e2f185d458aea4bd85d28c
EXPECTED_SNAP_SHA=8414fb2298bcd9797f5d1b0613d985413c7c87d533feb80e8f24360471c05b38
...
do
  ...
  case "$relative" in
    /*|.|..|../*|*/../*|*/..)
      printf 'RANK2-LATEST-AA1-NEXT ERROR: unsafe path for %s\n' \
        "$role" >&2
      exit 2
      ;;
  esac
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-latest-aa1-next.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-latest-modal-aa1-next-candidate.$$"
cleanup() {
  rm -rf "$WORK"
...
then
  printf '%s\n' \
    'RANK2-LATEST-AA1-NEXT ERROR: forbidden solver symbol.' >&2
  exit 2
fi
...
cp "$RUNNER" "$STAGE/run_rank2_latest_modal_aa1_next_candidate.sh"
...
(
  ...
  shasum -a 256 \
    ...
    run_rank2_latest_modal_aa1_next_candidate.sh classification.txt \
    > result.sha256
...
printf '%s\n' \
  "RANK2-LATEST-AA1-NEXT AX-SHA256=$ax_sha" \
  "RANK2-LATEST-AA1-NEXT SNAP-SHA256=$snap_sha" \
  'RANK2-LATEST-AA1-NEXT INPUTS-READ-ONLY HASH PASS' \
  'RANK2-LATEST-AA1-NEXT DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-LATEST-AA1-NEXT CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-LATEST-AA1-NEXT RESULT=$ARTIFACT_DIR"
