...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_modal_aa1_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-modal-aa1-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_modal_aa1_candidate.sh"
EXPECTED_AX_SHA=ae5f5b328fc6c5b181f40a4122b88771c857fd0200fc6e8351fc6b97d68d5c56
EXPECTED_SNAP_SHA=c11f4641897288f355ba60fa05eb7081d3aedfd8078333735d5c85c219f47c75
...
test "$(sed -n '1p' "$MANIFEST")" = \
  '# spot-rank2-modal-aa1-inputs-v1'
bad_width=$(awk 'NF && $1 !~ /^#/ && NF != 3 { count++ } \
  END { print count + 0 }' "$MANIFEST")
test "$bad_width" = 0
for role in x0 x1 x2 x2_snapshots basis_reference
do
  ...
  case "$relative" in
    /*|.|..|../*|*/../*|*/..)
      printf 'RANK2-MODAL-AA1 ERROR: unsafe path for %s\n' "$role" >&2
      exit 2
      ;;
  esac
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-modal-aa1.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-modal-aa1-candidate.$$"
cleanup() {
  rm -rf "$WORK"
...
then
  printf '%s\n' 'RANK2-MODAL-AA1 ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate x0.xsm x1.xsm x2.xsm x2_snapshots.xsm \
    proposal_axial.xsm proposal_snapshots.xsm > build.log
  ./check_candidate x0.xsm x1.xsm x2.xsm x2_snapshots.xsm \
    basis_reference.xsm proposal_axial.xsm proposal_snapshots.xsm \
    > check.log
)
...
cp "$RUNNER" "$STAGE/run_rank2_modal_aa1_candidate.sh"
...
(
  ...
  shasum -a 256 \
    ...
    run_rank2_modal_aa1_candidate.sh classification.txt \
    > result.sha256
...
printf '%s\n' \
  "RANK2-MODAL-AA1 AX-SHA256=$ax_sha" \
  "RANK2-MODAL-AA1 SNAP-SHA256=$snap_sha" \
  'RANK2-MODAL-AA1 INPUTS-READ-ONLY HASH PASS' \
  'RANK2-MODAL-AA1 DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-MODAL-AA1 CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-MODAL-AA1 RESULT=$ARTIFACT_DIR"
