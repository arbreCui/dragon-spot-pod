...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_modal_aa1_rolling_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-modal-aa1-rolling-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_modal_aa1_rolling_candidate.sh"
EXPECTED_AX_SHA=a7166bdfff6a477542118e5f355eacd4e0018c496717cac845df0cd6233108ee
EXPECTED_SNAP_SHA=6762e58a75cc2bcbffae476187389797a9aebf95e8619355060b6de9c41b5102
...
test "$bad_width" = 0
for role in aa1_pub aa1_plus xnext_pub xnext_plus \
...
do
  ...
  case "$relative" in
    /*|.|..|../*|*/../*|*/..)
      printf 'RANK2-ROLL-AA1 ERROR: unsafe path for %s\n' "$role" >&2
      exit 2
      ;;
  esac
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-roll-aa1.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-modal-aa1-rolling-candidate.$$"
cleanup() {
  rm -rf "$WORK"
...
for binding in aa1:aa1_pub aa1p:aa1_plus xnext:xnext_pub \
  xnextp:xnext_plus xnextps:xnext_plus_snapshots basis:basis_reference
do
  short=${binding%%:*}
  role=${binding#*:}
...
then
  printf '%s\n' 'RANK2-ROLL-AA1 ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate --rolling-aa1 aa1.xsm aa1p.xsm xnext.xsm \
    xnextp.xsm xnextps.xsm proposal_axial.xsm proposal_snapshots.xsm \
    > build.log
  ./check_candidate --rolling-aa1 aa1.xsm aa1p.xsm xnext.xsm \
    xnextp.xsm xnextps.xsm basis.xsm proposal_axial.xsm \
...
cp "$RUNNER" "$STAGE/run_rank2_modal_aa1_rolling_candidate.sh"
...
(
  ...
  shasum -a 256 \
    ...
    run_rank2_modal_aa1_rolling_candidate.sh classification.txt \
    > result.sha256
...
printf '%s\n' \
  "RANK2-ROLL-AA1 AX-SHA256=$ax_sha" \
  "RANK2-ROLL-AA1 SNAP-SHA256=$snap_sha" \
  'RANK2-ROLL-AA1 INPUTS-READ-ONLY HASH PASS' \
  'RANK2-ROLL-AA1 DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-ROLL-AA1 CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-ROLL-AA1 RESULT=$ARTIFACT_DIR"
