...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-map3-short.XXXXXX")
...
expected=$(awk '$2 == "basis_reference.xsm" { print $1 }' \
  "$ROOT/validation/iterative/one_map_scientific.sha256")
actual=$(shasum -a 256 "$BASIS_DIR/basis_reference.xsm" | awk '{ print $1 }')
test -n "$expected"
test "$actual" = "$expected"
cp "$BASIS_DIR/basis_reference.xsm" "$RADIAL_WORK/"

for name in state2_system.xsm state2_axial.xsm state2_snapshots.xsm
do
  expected=$(awk -v file="$name" '$2 == file { print $1 }' \
    "$X2_DIR/scientific.sha256")
  actual=$(shasum -a 256 "$X2_DIR/$name" | awk '{ print $1 }')
  test -n "$expected"
  test "$actual" = "$expected"
  cp "$X2_DIR/$name" "$RADIAL_WORK/$name"
done

cp "$ROOT/data/SpotRefFS.c2m" "$ROOT/data/SpotPlaneFS.c2m" \
  "$RADIAL_WORK/"
cp "$ROOT/validation/iterative/map3_radial.x2m" \
  "$RADIAL_WORK/radial.x2m"
cp "$ROOT/validation/iterative/map3_axial.x2m" \
...
reject_abnormal() {
  if rg -i 'FLU2DR-DIAG|CONVERGENCE NOT REACHED|XABORT|ABORT:|FATAL|SIG(SEGV|FPE|BUS|ILL|ABRT)|fortran runtime error' \
    "$1" >/dev/null
  then
    echo "MAP3-SHORT FAIL: abnormal or nonconverged solver marker." >&2
    exit 1
  fi
}

echo "MAP3-SHORT RADIAL START: 75 s process timeout + 5 s TERM grace, no retry"
...
test "$(rg -c '^SPOFCHK L2/MAX/RBAL/MIN/QSUM ' \
  "$RADIAL_WORK/radial.log")" = 3
test "$(rg -c '^>\|MAP3-RADIAL-COMPLETE' \
  "$RADIAL_WORK/radial.log")" = 1
echo "MAP3-SHORT RADIAL END"

for name in initial_axial_track.xsm initial_axial_macrolib.xsm \
  basis_reference.xsm state2_axial.xsm state3_system.xsm \
  state3_radial.xsm
do
  cp "$RADIAL_WORK/$name" "$AXIAL_WORK/$name"
done

echo "MAP3-SHORT AXIAL START: 75 s process timeout + 5 s TERM grace, no retry"
...
test "$(rg -c 'normal end of execution for dragon' \
  "$AXIAL_WORK/axial.log")" = 1
test "$(rg -c '^>\|MAP3-AXIAL-COMPLETE' \
  "$AXIAL_WORK/axial.log")" = 1
echo "MAP3-SHORT AXIAL END"

(
  cd "$AXIAL_WORK"
  ./check_one_map_xsm --continued basis_reference.xsm \
    state3_system.xsm state2_axial.xsm state3_axial.xsm \
    state3_snapshots.xsm
  shasum -a 256 state3_system.xsm state3_axial.xsm \
    state3_snapshots.xsm
)

rg 'MAP3-(RAW-DEFECT|STATE3)|SPOGBAL GLOBAL/MAX-GROUP' \
  "$AXIAL_WORK/axial.log"
shasum -a 256 "$DRAGON_BIN"
echo "MAP3-SHORT PASS: x3=G(x2) complete; convergence not assessed."
SUCCESS=1
