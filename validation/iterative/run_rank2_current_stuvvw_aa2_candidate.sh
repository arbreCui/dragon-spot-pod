...
MANIFEST=${MANIFEST:-"$ROOT/validation/iterative/rank2_current_stuvvw_aa2_candidate_inputs.tsv"}
ARTIFACT_DIR=${ARTIFACT_DIR:-"$ROOT/validation/artifacts/iterative-rank2-current-stuvvw-aa2-candidate"}
...
RUNNER="$ROOT/validation/iterative/run_rank2_current_stuvvw_aa2_candidate.sh"
EXPECTED_AX_SHA=2bfa87e6cfb0e1a4567e0fd210d5879a196ae2594f7fb7ce76ff8831860acf5e
EXPECTED_SNAP_SHA=9849404ff2a87ae886be47949c52e77f263447121116828f30dd306d29afa1d2
...
test "$(sed -n '1p' "$MANIFEST")" = \
  '# spot-rank2-current-stuvvw-aa2-candidate-inputs-v1'
test "$(awk 'NF && $1 !~ /^#/ {n++} END {print n+0}' "$MANIFEST")" = 7
test "$(awk 'NF && $1 !~ /^#/ && NF != 3 {n++} END {print n+0}' \
  "$MANIFEST")" = 0
for role in s_z_pub t u_aa2_pub v w w_snapshots basis_reference
do
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/spot-rank2-current-stuvvw-aa2.XXXXXX")
STAGE="$(dirname "$ARTIFACT_DIR")/.rank2-current-stuvvw-aa2.$$"
cleanup() {
  rm -rf "$WORK"
...
for binding in s:s_z_pub t:t u:u_aa2_pub v:v w:w ws:w_snapshots \
...
then
  printf '%s\n' 'RANK2-CURRENT-STUVVW-AA2 ERROR: forbidden solver symbol.' >&2
  exit 2
fi

(
  cd "$WORK"
  ./build_candidate --current-stuvvw-aa2 s.xsm t.xsm u.xsm v.xsm \
    v.xsm w.xsm ws.xsm proposal_axial.xsm proposal_snapshots.xsm \
    >build.log
  ./check_candidate --current-stuvvw-aa2 s.xsm t.xsm u.xsm v.xsm \
    v.xsm w.xsm ws.xsm basis.xsm proposal_axial.xsm \
    proposal_snapshots.xsm >check.log
)
rg -q '^RANK2-CURRENT-STUVVW-AA2 PARAMETER-FREE DIRECTION GATE PASS$' \
  "$WORK/build.log" "$WORK/check.log"
rg -q '^RANK2-CURRENT-STUVVW-AA2 CARRIER AA2-RAW-FLUX$' "$WORK/build.log"
rg -q '^RANK2-CURRENT-STUVVW-AA2 COMPLETE$' "$WORK/check.log"
...
cp "$RUNNER" "$STAGE/run_rank2_current_stuvvw_aa2_candidate.sh"
...
(
  ...
  shasum -a 256 proposal_axial.xsm proposal_snapshots.xsm build.log \
    ...
    run_rank2_current_stuvvw_aa2_candidate.sh classification.txt \
    >result.sha256
...
printf '%s\n' \
  "RANK2-CURRENT-STUVVW-AA2 AX-SHA256=$ax_sha" \
  "RANK2-CURRENT-STUVVW-AA2 SNAP-SHA256=$snap_sha" \
  'RANK2-CURRENT-STUVVW-AA2 INPUTS-READ-ONLY HASH PASS' \
  'RANK2-CURRENT-STUVVW-AA2 RECEIPT 9/9 PASS' \
  'RANK2-CURRENT-STUVVW-AA2 DRAGON/ASM/FLU/TRANSPORT=0' \
  'RANK2-CURRENT-STUVVW-AA2 CLASSIFICATION=MATERIALIZED_PROPOSAL_NOT_EVALUATED' \
  "RANK2-CURRENT-STUVVW-AA2 RESULT=$ARTIFACT_DIR"
