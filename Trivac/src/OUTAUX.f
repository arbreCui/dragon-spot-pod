...
*----
     ...
     1 ALBP,ESTOP,DEN3
      DOUBLE PRECISION, DIMENSION(:,:,:), ALLOCATABLE :: OUTSC
      DOUBLE PRECISION, DIMENSION(:,:), ALLOCATABLE :: OUTR,ACCUM
*----
*  DATA STATEMENT
*----
...
*----
      ...
      IGRDEB=IGRFIN+1
      IGRFIN=IGCOND(IGRC)
      OUTR(:NZS+1,:NREAC+4)=0.0D0
      OUTSC(:NZS,:NL+1,:NGCOND)=0.0D0
...
*----
     ...
     1   //'FOR FISSION SPECTRUM.')
         ...
         DEN(:NZS)=0.0
         IF(LENGT.EQ.0) THEN
            IF(IGR.EQ.IGRDEB) OUTR(:NZS,NREAC+3)=1.0D0
         ELSE
...
*----
      ...
      RATE(:NZS)=REAL(OUTR(:NZS,NREAC+2))
      CALL LCMPUT(KPMAC2,'FLUX-INTG',NZS,2,RATE)
      DO 400 IREAC=1,NREAC
      IF(LREAC(IREAC)) THEN
        DO 390 IBM=1,NZS
        RATE(IBM)=REAL(OUTR(IBM,IREAC))
        IF(RATE(IBM).NE.0.0) RATE(IBM)=RATE(IBM)/REAL(OUTR(IBM,NREAC+2))
  390   CONTINUE
  ...
  400 CONTINUE
      IF(LREAC(3)) THEN
        RATE(:NZS)=REAL(OUTR(:NZS,NREAC+3))
        CALL LCMPUT(KPMAC2,'CHI',NZS,2,RATE)
      ENDIF
      IF(LFIXE) THEN
        DO 410 IBM=1,NZS
        RATE(IBM)=REAL(OUTR(IBM,NREAC+4))
...
*
      ...
      IF(OUTSC(IBM,IL,JGRC).NE.0.0D0) THEN
  ...
  430 CONTINUE
      ...
      IPOSDE=IPOSDE+1
      SCAT(IPOSDE)=REAL(OUTSC(IBM,IL,JGRC))
  440 CONTINUE
  450 CONTINUE
      ...
      RATE(:NZS)=REAL(OUTSC(:NZS,IL,IGRC))
      CALL LCMPUT(KPMAC2,'SIGW'//SUFF,NZS,2,RATE)
  460 CONTINUE
      DEALLOCATE(SCAT)
      DEALLOCATE(IJJ,NJJ,IPOS)
*
  ...
  470    CONTINUE
         DO 480 IAL=1,NALBP
         ALBP(IAL,IGRC)=ALBP(IAL,IGRC)/REAL(DFI)
  480    CONTINUE
      ENDIF
*----
