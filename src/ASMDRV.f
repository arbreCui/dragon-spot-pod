*DECK ASMDRV
     ...
     >                  VOLUME,LEAKSW,ITRANC,LDIFF,IBFP,ISPOT,TITRE,
...
* IPSNAP  pointer to the archive LCM object containing snapshots.
...
* ISPOT   snapshot index for the spot method.
...
*----
     ...
     >            MATCOD(NREGIO),ITRANC,IBFP,ISPOT,ITPIJ,IPHASE,ISTRM,
     >            KNORM,NALBP
      REAL        VOLUME(NREGIO)
*----
*  LOCAL VARIABLES
*----
      ...
      REAL, ALLOCATABLE, DIMENSION(:) :: TEMP,ENERGY,DB2
...
*----
     ...
     >                      IREGIO=1,NREGIO)
      ENDIF
      IF(ISPOT.GT.0) THEN
        IF(.NOT.C_ASSOCIATED(IPSNAP)) CALL XABORT('ASMDRV: IPSNAP IS N'
     1  //'OT DEFINED.')
        JPSNAP=LCMGID(IPSNAP,'FLUX')
        KPSNAP=LCMGIL(JPSNAP,ISPOT)
        ALLOCATE(DB2(NGROUP))
        CALL LCMGET(KPSNAP,'SPOT-LEAK1D',DB2)
      ENDIF
...
*         PERFORM A P0_TOTAL LEAKAGE CORRECTION.
   ...
   55     CONTINUE
        ENDIF
*----
*  INCLUDE 1D (AXIAL) LEAKAGE FOR THE SPOT METHOD.
*----
        IF(ISPOT.GT.0) THEN
          XSSIGW(:NBMIX,1,IGR)=XSSIGW(:NBMIX,1,IGR)-DB2(IGR)
        ENDIF
*
        IF(IPRNTP.GE.3) THEN
