*DECK ASMDRV
      SUBROUTINE ASMDRV(IPSYS,IPTRK,IPMACR,IPSNAP,IFTRAK,CDOOR,IPRNTP,
     >                  NGROUP,NBMIX,NREGIO,NANI,NANIST,NW,MATCOD,
     >                  VOLUME,LEAKSW,ITRANC,LDIFF,IBFP,ISNAP_L1,TITRE,
     >                  ITPIJ,LNORM,IPHASE,ISTRM,KNORM,NALBP)
*
*-----------------------------------------------------------------------
*
*Purpose:
...
* IPSNAP  pointer to the optional SPOT snapshot archive.
...
* ISNAP_L1 snapshot index supplying LEAK1D (=0: no LEAK1D).
...
*----
      ...
      TYPE(C_PTR) IPSYS,IPTRK,IPMACR,IPSNAP
      INTEGER     IFTRAK,IPRNTP,NGROUP,NBMIX,NREGIO,NANI,NANIST,NW,
     >            MATCOD(NREGIO),ITRANC,IBFP,ISNAP_L1,ITPIJ,IPHASE,
     >            ISTRM,KNORM,NALBP
      REAL        VOLUME(NREGIO)
*----
*  LOCAL VARIABLES
*----
      ...
      TYPE(C_PTR) JPSYS,KPSYS,JPMACR,KPMACR,JPSNAP,KPSNAP
      REAL, ALLOCATABLE, DIMENSION(:) :: TEMP,ENERGY,LEAK1D
      REAL, ALLOCATABLE, DIMENSION(:,:) :: XSSCOR,XSDIFF,ALBP,EMOMTR
      REAL, ALLOCATABLE, DIMENSION(:,:) :: S0PHYS
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: LEAK1D64
      LOGICAL LDPL1D
      INTEGER ILONGD,ITYLCD,IMIRL,L1RAW
...
*----
     ...
     >                      IREGIO=1,NREGIO)
      ENDIF
      ALLOCATE(LEAK1D(NGROUP),S0PHYS(0:NBMIX,NGROUP))
      ALLOCATE(LEAK1D64(NGROUP))
      LEAK1D(:NGROUP)=0.0
      LEAK1D64(:NGROUP)=0.0D0
      LDPL1D=.FALSE.
      S0PHYS(0:NBMIX,:NGROUP)=0.0
      IF(ISNAP_L1.GT.0) THEN
        IF(.NOT.C_ASSOCIATED(IPSNAP)) CALL XABORT('ASMDRV: SNAPSHOT AR'
     1  //'CHIVE IS NOT DEFINED.')
        JPSNAP=LCMGID(IPSNAP,'FLUX')
        KPSNAP=LCMGIL(JPSNAP,ISNAP_L1)
        ...
        ELSE IF(ILONG.NE.0) THEN
          CALL XABORT('ASMDRV: INVALID SPOT-LEAK1D LENGTH.')
        ENDIF
        DO IGR=1,NGROUP
          IF((LEAK1D(IGR).NE.LEAK1D(IGR)).OR.
     1      (ABS(LEAK1D(IGR)).GT.HUGE(LEAK1D(IGR))))
     1      CALL XABORT('ASMDRV: NON-FINITE SPOT-LEAK1D.')
        ENDDO
*       PRESENCE-GATED REAL64 LEAKAGE AUTHORITY: WHEN THE SNAPSHOT
*       PLANE CARRIES LEAK1D64, THE REAL32 RECORD MUST BE ITS EXACT
*       BITWISE DEMOTE MIRROR AND THE SELF-SCATTERING REDUCTION IS
*       DEFERRED TO THE REAL64 SOLVER CORE (SYSTEM STAYS PHYSICAL).
        CALL LCMLEN(KPSNAP,'LEAK1D64',ILONGD,ITYLCD)
        IF(ILONGD.EQ.NGROUP) THEN
          IF(ITYLCD.NE.4) CALL XABORT(
     1      'ASMDRV: INVALID LEAK1D64 TYPE.')
          CALL LCMGET(KPSNAP,'LEAK1D64',LEAK1D64)
          DO IMIRL=1,NGROUP
            IF(TRANSFER(REAL(LEAK1D64(IMIRL)),0).NE.
     1         TRANSFER(LEAK1D(IMIRL),0)) CALL XABORT(
     2        'ASMDRV: LEAK1D64 MIRROR MISMATCH.')
          ENDDO
          LDPL1D=.TRUE.
        ELSE IF(ILONGD.NE.0) THEN
          CALL XABORT('ASMDRV: INVALID LEAK1D64 LENGTH.')
        ENDIF
        CALL LCMPUT(IPSYS,'SPOT-LEAK1D',NGROUP,2,LEAK1D)
        CALL LCMPUT(IPSYS,'SPOT-L1-SNAP',1,1,ISNAP_L1)
*       MARK AN UNREDUCED SYSTEM.  ITS SELF-SCATTERING STILL CARRIES
*       THE PHYSICAL VALUE AND THE LEAKAGE IS APPLIED INSIDE THE
*       REAL64 SOLVER CORE; ANY OTHER SOLVER WOULD SILENTLY DROP IT.
        IF(LDPL1D) THEN
          L1RAW=1
          CALL LCMPUT(IPSYS,'SPOT-L1-RAW',1,1,L1RAW)
        ENDIF
        IF(IPRNTP.GE.1) WRITE(IUNOUT,6202) ISNAP_L1,
     1    MINVAL(LEAK1D),MAXVAL(LEAK1D)
      ENDIF
...
*         PERFORM A P0_TOTAL LEAKAGE CORRECTION.
   ...
   55     CONTINUE
        ENDIF
*----
*  APPLY POSITIVE-LOSS LEAK1D ONCE AS A SELF-SCATTERING REDUCTION.
*  KEEP THE PHYSICAL P0 SCATTERING FOR THE LATER AXIAL PROJECTION.
*----
        S0PHYS(0:NBMIX,IGR)=XSSIGW(0:NBMIX,1,IGR)
        IF(ISNAP_L1.GT.0.AND..NOT.LDPL1D) THEN
...
*----
      ...
      IF(ISNAP_L1.GT.0) CALL LCMPUT(KPSYS,'SPOT-S0-PHYS',NBMIX+1,2,
     > S0PHYS(0,IGR))
...
*----
 ...
 6201 FORMAT(//' COMPUTATION OF DRAGON COMPLETE CP BY DOOR =',
     >3X,A12)
 6202 FORMAT(/' ASMDRV: APPLY LEAK1D FROM SNAPSHOT',I4,
     > ' (MIN/MAX=',1P,2E12.4,')')
      END
