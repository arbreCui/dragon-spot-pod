...
*     SYSTEM and SOURCE records.  No new steering parameter is introduced.
     ...
     1   C_ASSOCIATED(IPSYS).AND.C_ASSOCIATED(IPSOU)) THEN
         CALL LCMLEN(IPSYS,'SPOT-L1-RAW',ILEN,ITYLCM)
         IF(ILEN.NE.0) CALL XABORT(
     1   'FLU2DR: UNREDUCED SYSTEM REQUIRES THE REAL64 DOOR.')
...
*----
      DEALLOCATE(LEAK1D,XCSOU,DIFHET,DHOM,GAMMA,XSCAT,FLUX)
