...
SUBROUTINE MUSACG(ITRACK,IFTRK,IPRINT,IMACRO,NBSLIN,RCUTOF,GG,LGINF,NBNODE_MACRO,NSURF_MACRO)
  ...
  !----
  IF(NPERIM.GT.20) THEN
    WRITE(FOUT,'(A,I5,A,I5,A)') ' MUSACG: NPERIM=',NPERIM, &
      ' NB_ELEM=',GG_MAC%NB_ELEM,' -> capping IFOLD_GLOB at 2**20'
  ENDIF
  ALLOCATE(IFOLD_GLOB(INT(2_8**MIN(NPERIM,20))*GG_MAC%NB_ELEM))
