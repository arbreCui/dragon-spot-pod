 ...
 10   CONTINUE
      ...
      IF(REC) THEN
         ...
         IF((ILCML1.NE.NSTATE).OR.(ITYLCM.NE.1)) THEN
           CALL XABORT('FLUGPI: INVALID STATE-VECTOR RECORD.')
           RETURN
         ENDIF
         ...
         MAXOUT=ISTATE(12)
         NMERG=ISTATE(18)
         CALL LCMLEN(IPFLUX,'EPS-CONVERGE',ILCML1,ITYLCM)
         IF((ILCML1.NE.5).OR.(ITYLCM.NE.2)) THEN
           CALL XABORT('FLUGPI: INVALID EPS-CONVERGE RECORD.')
           RETURN
         ENDIF
     ...
     >      ((ILCML2.EQ.1).AND.(ITYLCM.EQ.2)))) THEN
         ...
         ENDIF
         IF(NMERG.GT.0) THEN
           CALL LCMLEN(IPFLUX,'IMERGE-LEAK',ILCML1,ITYLCM)
           IF((ILCML1.NE.NMAT).OR.(ITYLCM.NE.1)) THEN
             CALL XABORT('FLUGPI: INVALID IMERGE-LEAK RECORD.')
             RETURN
           ENDIF
           CALL LCMGET(IPFLUX,'IMERGE-LEAK',IMERG)
         ENDIF
      ELSE
         ITYPEC=-99
         ILEAK=0
...
*         use DSA flux
 ...
 130     CONTINUE
     ...
     >      ((ILCMLN.EQ.3).AND.(ITYLCM.EQ.2)))) THEN
           CALL XABORT('FLUGPI: INVALID MACRO B2  HETE RECORD.')
           RETURN
         ENDIF
         IF(ILCMLN.EQ.3) THEN
     ...
     >      ((ILCMLN.EQ.1).AND.(ITYLCM.EQ.2)))) THEN
           CALL XABORT('FLUGPI: INVALID MACRO B2  B1HOM RECORD.')
           RETURN
         ENDIF
         IF(ILCMLN.EQ.1) THEN
