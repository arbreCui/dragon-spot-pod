*DECK SPOF64
      SUBROUTINE SPOF64(KPSYS,IPTRK,IMPX,NGEFF,NGIND,NREG,NMAT,NUN,
     1 MAT,KEYFLX,FUNKNO,SUNKNO,TITR)
*
*-----------------------------------------------------------------------
*
*Purpose:
* Solve the coupled axial POD equations and reconstruct the regional
* flux for the 2D/1D Synthesis Proper Orthogonal Decomposition method.
*
*Copyright:
...
*----
     ...
     >            KEYFLX(NREG)
      DOUBLE PRECISION FUNKNO(NUN,NGEFF),SUNKNO(NUN,NGEFF)
*----
*  LOCAL VARIABLES
*----
...
*----
      ...
      REAL, ALLOCATABLE, DIMENSION(:) :: VOL1D,VOL2D,GAR
      REAL, ALLOCATABLE, DIMENSION(:,:) :: SGAR,BASIS,RADIAL2D
      REAL, ALLOCATABLE, DIMENSION(:,:,:) :: SGAS
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:) :: RADIAL2D64
...
*----
     ...
     1 6H WITH ,A,1H.)') IGR,'SPOT'
      ...
      CALL LCMGET(KPSYS(II),'POD-NMODE',NMODE)
      IF(NMODE.LE.0) CALL XABORT('SPOF: INVALID POD MODE COUNT.')
      ALLOCATE(BASIS(NREG2D,NMODE),VOL2D(NREG2D))
      ALLOCATE(RADIAL2D(NREG2D,NSNAP),RADIAL2D64(NREG2D,NSNAP))
      CALL LCMGET(KPSYS(II),'POD-BASIS',BASIS)
      CALL LCMGET(KPSYS(II),'VOL2D',VOL2D)
      CALL LCMGET(KPSYS(II),'RADIAL-OP',RADIAL2D)
*     THE REAL64 RADIAL OPERATOR IS THE AUTHORITY WHEN PRESENT; THE
*     REAL32 RECORD MUST THEN BE ITS EXACT BITWISE DEMOTE MIRROR.
      CALL LCMLEN(KPSYS(II),'RADIAL-OP64',ILONG64,ITY64)
      IF(ILONG64.EQ.0) THEN
        RADIAL2D64(:,:)=DBLE(RADIAL2D(:,:))
      ELSE
        IF(ILONG64.NE.NREG2D*NSNAP) CALL XABORT(
     1    'SPOF: INVALID RADIAL-OP64 LENGTH.')
        IF(ITY64.NE.4) CALL XABORT('SPOF: INVALID RADIAL-OP64 TYPE.')
        CALL LCMGET(KPSYS(II),'RADIAL-OP64',RADIAL2D64)
        DO ISNAP=1,NSNAP
          DO IRG=1,NREG2D
            IF(TRANSFER(REAL(RADIAL2D64(IRG,ISNAP)),0).NE.
     1         TRANSFER(RADIAL2D(IRG,ISNAP),0)) CALL XABORT(
     2        'SPOF: RADIAL-OP64 MIRROR MISMATCH.')
          ENDDO
        ENDDO
      ENDIF
*----
...
*----
      ...
      CALL SPOT1P64(NREG2D,NFLOOR,IELEM,NMAT,NSNAP,NMODE,ISCHM,
     1 NLF,NSCT,LFIXUP,MAT1D,VOL1D,MAT,SGAR(0,II),SGAS(0,1,II),
     2 BASIS,VOL2D,RADIAL2D64,NCODE,ZCODE,SUNKNO(1,II),DU,W,PL,
     3 FUNKNO(1,II),FUNKNO(LL4+1,II),FUNKNO(LL4+LL5+1,II))
*----
* END OF LOOP OVER ENERGY GROUPS
*----
      DEALLOCATE(RADIAL2D64,RADIAL2D,VOL2D,BASIS)
  130 CONTINUE ! II
*----
*  PRINT CPU TIME
*----
