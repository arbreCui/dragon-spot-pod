...
CONTAINS
  ...
  SUBROUTINE SAL100(GG)
    ...
    SELECT CASE(TYPGEO)
       CASE(1)
       IF(NBFOLD.EQ.3) THEN
         ANGGEO=TWOPI/8.0D0 ! half diagonal symmetry case
       ELSE
         ANGGEO=TWOPI/NBFOLD
       ENDIF
       CASE(2)
       ...
       ANGGEO=PI/6.
    END SELECT
    IF((PRTIND >= 1).AND.(TYPGEO.EQ.0)) THEN
      WRITE(FOUT,*) 'SAL100: TYPGEO=',TYPGEO,' NBFOLD=',NBFOLD
    ELSE IF(PRTIND >= 1) THEN
      WRITE(FOUT,*) 'SAL100: TYPGEO=',TYPGEO,' NBFOLD=',NBFOLD, &
      & ' ANGGEO=',ANGGEO,' radians'
    ENDIF
  ...
  SUBROUTINE SAL110(GG)
    ...
    !---------------------------------------------------------------------
    !
    USE SAL_GEOMETRY_TYPES, ONLY : TYPGEO,NBFOLD,NIPAR,NRPAR,ALLSUR,NANIS,IC,ISPEC,ANGGEO
    USE SAL_TRACKING_TYPES, ONLY : PRTIND
    !****
    IMPLICIT NONE
    ! in variable
    ...
    IF((TYPGEO==0).OR.((TYPGEO==6).AND.(NBFOLD==0))) THEN
      CONTINUE
    ELSE IF((TYPGEO==1).AND.(NBFOLD==3)) THEN
      HSYM='QUAR'
    ...
    ELSE IF((TYPGEO==1).AND.(NBFOLD==2)) THEN
      HSYM='SY1D'
    ELSE IF(((TYPGEO==2).AND.(NBFOLD==6)).OR.((TYPGEO==10).AND.(NBFOLD==0))) THEN
      HSYM='RA60'
    ELSE IF((TYPGEO==5).AND.(NBFOLD==0)) THEN
      HSYM='TRAN'
    ELSE IF((TYPGEO==9).AND.(NBFOLD==0)) THEN
      HSYM='TRAN'
    ELSE IF(((TYPGEO==2).AND.(NBFOLD==3)).OR.(TYPGEO==11).AND.(NBFOLD==0)) THEN
      HSYM='R120'
    ELSE
    ...
    IF((ISPEC==0).AND.(IC.EQ.4)) THEN
      ...
      ELSE IF(HSYM=='SY1D') THEN
        ...
        IF(NANIS>1) CALL XABORT('SAL110: unfold unsupported with NANIS>1')
        CALL SALFOLD_1('SYMX',GG)
      ELSE IF(HSYM=='SIXT') THEN
        IF(PRTIND>0) WRITE(*,*) "SAL110: SA60 unfold"
        IF(NANIS>1) CALL XABORT('SAL110: unfold unsupported with NANIS>1')
      ...
      ELSE IF(HSYM=='RA60') THEN
        ...
        IF(NANIS>1) CALL XABORT('SAL110: unfold unsupported with NANIS>1')
        CALL SALFOLD_2('SR60',GG)
        CALL SALFOLD_2('R180',GG)
      ELSE IF(HSYM=='R120') THEN
        IF(PRTIND>0) WRITE(*,*) "SAL110: SR120 unfold with rotation"
        IF(NANIS>1) CALL XABORT('SAL110: unfold unsupported with NANIS>1')
        CALL SALFOLD_2('R120',GG)
      ELSE
  ...
  SUBROUTINE SALINP(GG)
    ...
    IF(GG%NBBCDA>0)THEN
       ...
       DO I=1,GG%NBBCDA
          ...
          IF(TYPE>5.OR.TYPE<0) CALL XABORT('SALINP: wrong bc type')
          IF(NBER>GG%NB_ELEM) CALL XABORT('SALINP: bc def exceeds nber of elements')
          !
          ...
          ELSE
            ...
            ELSE IF(TYPGEO.EQ.12) THEN
              IF((ANGLE/=0.0).AND.(ANGLE/=PI/6._PDB).AND.(ANGLE/=PI/3._PDB).AND.(ANGLE/=2._PDB*PI/3._PDB)) THEN
                WRITE(HSMG,100) ANGLE,I,TYPGEO
                CALL XABORT(HSMG)
              ENDIF
            ENDIF
  ...
  SUBROUTINE SAL128(RPAR,IPAR,NB_ELEM)
    ...
    SELECT CASE(TYPGEO)
       ...
       CASE(10) 
       CALL SAL128_7(NN,POINT,EPS,EPS2)
       CASE(11) 
       CALL SAL128_8(NN,POINT,EPS,EPS2)
    END SELECT
...
10           IP(J)=IP(J)+1
  ...
  !
  SUBROUTINE SAL128_7(NN,POINT,EPS,EPS2)
    !
    !---------------------------------------------------------------------
    !
    !Purpose:
    ! process an isocel trianglar geometry (TYPGEO=10) with RA60 rotation:
    ! adjusts points on the axes, computes the triangular side length
    !
    !Parameters: input
    ! NN        total number of points
    ...
    !
    USE PRECISION_AND_KINDS,    ONLY : PDB
    USE SAL_GEOMETRY_TYPES,     ONLY : ANGGEO,LX=>LENGTHX,LY=>LENGTHY
    !****
    IMPLICIT NONE
    INTEGER,   INTENT(IN)     :: NN
    REAL,      INTENT(IN)     :: EPS,EPS2
    ...
    ENDDO ITER0
    !
  END SUBROUTINE SAL128_7
  !
  SUBROUTINE SAL128_8(NN,POINT,EPS,EPS2)
    !
    !---------------------------------------------------------------------
    !
    !Purpose:
    ! process R120 lozenge geometry (TYPGEO=11): adjusts points on the axes,
    ! computes the lozenge side length
    !
    !Parameters: input
    ! NN        total number of points
    ! EPS       when distance of points to axes less than EPS, displace
    ...
    !****
    INTEGER   :: NP,NAXIS,I,P
    REAL(PDB) :: D,AUX,EX(4),EY(4),DIS(4),AX(4),AY(4)
    INTEGER, PARAMETER :: FOUT =6
    !****
    !     compute the triangular side
    LX=0.; LY=0.
    DO P=1,NN
       ...
       IF(LY<POINT(2,P)) LY=POINT(2,P)
    ENDDO
    IF(ABS(LY-LX*SIN(ANGGEO))>EPS) &
         WRITE(FOUT,'(" warning: h is different from a*sin(anggeo)!", &
         &  " a = ",E12.4," h = ",E12.4)') LX,LY
    !     adjust h according to base
    LY=LX*SIN(ANGGEO)
    !*    adjust points on the axes
    NAXIS=4
    !     axis directions
    EX(1)=1.; EY(1)=0.; DIS(1)=0.
    EX(2)=1.; EY(2)=0.; DIS(2)=-LY
    EX(3)=COS(ANGGEO); EY(3)=SIN(ANGGEO); DIS(3)=0.
    EX(4)=COS(ANGGEO); EY(4)=SIN(ANGGEO); DIS(4)=LY
    NP=4
    !     axis ends
    AX(1)=0.    ; AY(1)=0.
    AX(2)=LX*0.5; AY(2)=LY
  ...
  SUBROUTINE SAL130_2(NB_ELEM,NB_NODE,IPAR,PPERIM,PERIM,LIST)
    ...
    DO NODE=1,NB_NODE
       ...
       IF(NN>0) THEN
          PPERIM(NODE+1)=NT+1
       ELSE
          WRITE(HSMG,'(15HSAL130_2: node=,i5,19H without perimeter.)') NODE
          CALL XABORT(HSMG)
       ENDIF
    ENDDO
    IF(NT>0) THEN
  ...
  SUBROUTINE SAL130_4(NB_ELEM,NB_BC,IPAR,IBC2_ELEM,LIST_BC)
    ...
    DO ELEM=1, NB_ELEM
       ...
       ELSE IF(IPAR(2,ELEM)<=0.OR.IPAR(3,ELEM)<=0) THEN
          NB_BC=NB_BC+1
          LIST_BC(NB_BC)=ELEM
          IF(IBC2_ELEM(ELEM)/=0) CALL XABORT('SAL130_4: two surfaces to element')
  ...
  SUBROUTINE SAL130_10(NPERIM,PERIM,PPERIM,DIST_AXIS,IBC2_ELEM,TYPE_BC2,IDATA_BC2, &
    ...
    !
    !Purpose:
    ! calculate PERIM_MAC2 and DIST_AXIS for the cyclical geometries:
    ...
    DO I=1,NPERIM
       ...
       SELECT CASE(TYPGEO)
          ...
          ENDIF
          CASE(10)
          !*          triangles with rotations and translations:
          !           typgeo=10 axes definition:
          !                        *
          !                M=2   *   *    M=3
          !                    *       *
          !                  *************
          !                        M=1
          IF((TYPBC==G_BC_TYPE(2)).OR.(TYPBC==G_BC_TYPE(3))) THEN
             IF(ABS(ANGLE)<EPS) THEN
                !                 element is on the axis 1 (angle=0)
                M=1
             ELSEIF(ABS(ANGLE-ANGGEO)<EPS) THEN
                ...
                M=2
             ELSEIF(ABS(ANGLE-2.*ANGGEO)<EPS) THEN
                !                 element is on the axis 3 (angle=2*anggeo)
                M=3
             ELSE
                CALL XABORT('SAL130_10: boundary condition data error in element for TYPGEO=10.')
             ENDIF
          ELSE
             CALL XABORT('SAL130_10: wrong boundary condition for TYPGEO=10.')
          ENDIF
          CASE(11)
          !*          lozenge with rotations and translations:
          !           axes definition:
          !                          M=3
          !                     ************
          !                    *          *
          ...
          IF((TYPBC==G_BC_TYPE(2)).OR.(TYPBC==G_BC_TYPE(3))) THEN
             ...
             ELSE
                IF(BCDATA(1,IDATA)>0) THEN
                   !                    cx>0: element is on the axis 4 (angle>0)
                   M=4
                ELSE
                   !                    cx=0: element is on the axis 2 (angle>0)
                   M=2
                ENDIF
             ENDIF
          ELSE
             CALL XABORT('SAL130_10: TYPGEO=11:wrong boundary condition type.')
          ENDIF
          CASE DEFAULT
    ...
    !     nber of blocks in 'perim'
    M=NAXES
    !*    organize elements as: (elems on axis 1)+(elems on axis 2)+...
    PPERIM(1)=1
    DO I=1,M
       PPERIM(I+1)=PPERIM(I)+NBE(I)
    ENDDO
  ...
  SUBROUTINE SAL131(GG)
    ...
    REAL(PDB), DIMENSION(G_BC_MAX_LEN,GG%NB_ELEM+5) :: TMP_BCDATA
    !*****
    !*    initialization
       ...
       DO I=1,GG%NBBCDA
          ...
          SELECT CASE(TYPE)
             ...
             SELECT CASE(TYPGEO)
                ...
                IF(TMP_BCDATA(1,ITBC)/=0) TMP_BCDATA(1,ITBC)=SIGN(LENGTHX,TMP_BCDATA(1,ITBC))
                IF(TMP_BCDATA(2,ITBC)/=0) TMP_BCDATA(2,ITBC)=SIGN(LENGTHY,TMP_BCDATA(2,ITBC))
                CASE(9)
                ...
                !                adjust translation data according to the hexagonal sides
                IF(ABS(ANGLE)<EPS) THEN
                   IF(TMP_BCDATA(2,ITBC)>0.) THEN
                     !                    cy>0 and angle=0
                     TMP_BCDATA(1,ITBC)=LENGTHX/2.0
                     TMP_BCDATA(2,ITBC)=LENGTHY
                   ELSE
                     !                    cy=0 and angle=0
                     TMP_BCDATA(1,ITBC)=0.0
                     TMP_BCDATA(2,ITBC)=0.0
                   ENDIF
                ELSE
                   IF(TMP_BCDATA(1,ITBC)>0.) THEN
                     !                    cx>0 and angle=pi/3 rad
                     TMP_BCDATA(1,ITBC)=LENGTHX
                     TMP_BCDATA(2,ITBC)=0.0
                   ELSE
                     !                    cx=0 and angle=pi/3 rad
                     TMP_BCDATA(1,ITBC)=0.0
       ...
       DO I=1,2
          IF(IDATA(I)==0) THEN
             ...
             ELSEIF(TYPGEO==2) THEN
                ...
                ENDIF
             ENDIF
             TMP_BCDATA(1,ITBC)=0._PDB
             TMP_BCDATA(2,ITBC)=0._PDB
             TMP_BCDATA(3,ITBC)=COS(ANGLE)
             TMP_BCDATA(4,ITBC)=SIN(ANGLE)
             TMP_BCDATA(5,ITBC)=ANGLE
             IDATA(I)=ITBC
  ...
  SUBROUTINE SAL131_2(NB_ELEM,DEFAUL,IPAR,IBC2_ELEM,TYPE_BC2,IDATA_BC2)
    ...
    DO ELEM=1,NB_ELEM
       ...
       IF(LGBC) THEN
          ...
          !           put position of "defaul" in bcdata table (use default albedo)
          IDATA_BC2(IBC)=0
       ENDIF
    ENDDO
    !
  ...
  SUBROUTINE SAL160_2(NB_ELEM,IPAR,RPAR,VOL2,ISURF2_ELEM,NB_SURF2,SURF2)
    ...
    DO ELEM=1,NB_ELEM
       ...
       NODEBC=IPAR(2,ELEM)
       LGBC=NODEBC<=0
       IF(.NOT.LGBC) VOL2(NODEBC)=VOL2(NODEBC)+AUX
       NODE=IPAR(3,ELEM)
       IF(NODE>0)THEN
       ...
       ENDIF
       IF(LGBC) THEN
          IF(NODEBC==G_BC_TYPE(-1).OR.NODEBC==G_BC_TYPE(0).OR.NODEBC==G_BC_TYPE(1))THEN
  ...
  !
  SUBROUTINE SALROT(X0,Y0,THETA,X3,Y3,X4,Y4)
    !
    !---------------------------------------------------------------------
    !
    !Purpose:
    ! set the symmetric point (x4,y4) of (x3,y3) relative to the rotation
    ! center (x1,y1) with angle theta.
    !
    !Parameters: input
    ! X0    abscissa coordinate of the rotation center
    ! Y0    ordinate coordinate of the rotation center
    ! THETA rotation angle
    ! X3    abscissa coordinate of the point to mirror
    ! Y3    ordinate coordinate of the point to mirror
    !
    !Parameters: output
    ! X4    abscissa coordinate of the symmetric point
    ! Y4    ordinate coordinate of the symmetric point
    ...
    !
    IMPLICIT NONE
    REAL(PDB),INTENT(IN)  :: X0,Y0,THETA,X3,Y3
    REAL(PDB),INTENT(OUT) :: X4,Y4

    REAL(PDB) :: A,B
    IF(THETA==0.0_PDB) THEN
      X4=X3; Y4=Y3
    ELSE
      A=X3-X0; B=Y3-Y0
      X4=X0+A*COS(THETA)-B*SIN(THETA); Y4=Y0+A*SIN(THETA)+B*COS(THETA)
    ENDIF
  ...
  SUBROUTINE SALFOLD_0(GG,IPASS,IB,NBBCDA,ALIGN,LFOLD,IFOLD)
    ...
    DO ELEM=1,GG%NB_ELEM
      IF(I2(1,ELEM).GT.2*GG%NB_ELEM) CALL XABORT('SALFOLD_0: IFOLD overflow')
  ...
  SUBROUTINE SALFOLD_1(HSYM,GG)
    ...
    !
    !Purpose:
    ! unfold the domain with reflection
    !
    !Parameters: input
    ...
    !
    NSYM=1
    IF(HSYM=='SYMX') THEN ! define symmetry axis
      AXIS_X1(1)=0._PDB; AXIS_X2(1)=100._PDB; AXIS_Y1(1)=0._PDB; AXIS_Y2(1)=0._PDB;
    ELSE IF(HSYM=='SYMY') THEN
      AXIS_X1(1)=0._PDB; AXIS_X2(1)=0._PDB; AXIS_Y1(1)=0._PDB; AXIS_Y2(1)=100._PDB;
    ...
    DO ELEM=1,GG%NB_ELEM
      ...
      DO ISYM=1,NSYM
        ...
        ELSE IF(TYPE==3) THEN
          CALL SALSYM(AXIS_X1(ISYM),AXIS_Y1(ISYM),AXIS_X2(ISYM),AXIS_Y2(ISYM),X1,Y1,X4,Y4)
          X1B=X1+RAD*COS(GG%RPAR(4,ELEM)); Y1B=Y1+RAD*SIN(GG%RPAR(4,ELEM));
          CALL SALSYM(AXIS_X1(ISYM),AXIS_Y1(ISYM),AXIS_X2(ISYM),AXIS_Y2(ISYM),X1B,Y1B,X4B,Y4B)
          IF((ABS(X4B-X4)<EPS*ABS(RAD)).AND.(Y4B-Y4 > 0._PDB)) THEN
            THETA1=PI/2._PDB
          ELSE IF((ABS(X4B-X4)<EPS*ABS(RAD)).AND.(Y4B-Y4 < 0._PDB)) THEN
            THETA1=3._PDB*PI/2._PDB
    ...
    ! loop over boundary conditions
    TMP_NBBCDA=0
    ALLOCATE(ISPERIM(GG%NB_ELEM),ALIGN(3,3,GG%NB_ELEM),ANGLE(GG%NB_ELEM), &
    & ALBEDO(GG%NB_ELEM),PERIM_ELEM(GG%NB_ELEM))
    ALIGN(:3,3,:GG%NB_ELEM)=1.0_PDB
    PERIM_ELEM(:GG%NB_ELEM)=0
    ISPERIM(:GG%NB_ELEM)=.FALSE.
    DO IBC=1,GG%NBBCDA
      DO I=1,GG%BCDATAREAD(IBC)%NBER
        ...
        IF(INDBC==0) CYCLE
        DO ISYM=1,NSYM+1
          IF(I2(ISYM,INDBC)/=0) ISPERIM(I2(ISYM,INDBC))=.TRUE.
        ENDDO
      ENDDO
    ENDDO
    ITER0: DO ELEM=1,GG%NB_ELEM
      ...
      ANGLE(TMP_NBBCDA)=ATAN((Y2-Y1)/(X2-X1))
      IF(ABS(ANGLE(TMP_NBBCDA)).LE.1.0E-5) ANGLE(TMP_NBBCDA)=0.0
      ALIGN(1,1,TMP_NBBCDA)=X1; ALIGN(1,2,TMP_NBBCDA)=Y1
      ALIGN(2,1,TMP_NBBCDA)=X2; ALIGN(2,2,TMP_NBBCDA)=Y2
      ! Recover albedo from folded geometry
      ALBEDO(TMP_NBBCDA)=1.0
      DO IBC=1,GG%NBBCDA
        J = FINDLC(GG%BCDATAREAD(IBC)%ELEMNB,ELEM)
    ...
    DO IBC=1,TMP_NBBCDA
      ...
      DO I=1,GG%NB_ELEM
        ...
        ENDIF
      ENDDO
      TMP_BCDATAREAD(IBC)%BCDATA(1) = ALIGN(1,1,IBC)
      TMP_BCDATAREAD(IBC)%BCDATA(2) = ALIGN(1,2,IBC)
      TMP_BCDATAREAD(IBC)%BCDATA(3) = COS(ANGLE(IBC))
      TMP_BCDATAREAD(IBC)%BCDATA(4) = SIN(ANGLE(IBC))
      TMP_BCDATAREAD(IBC)%BCDATA(5) = ANGLE(IBC)
      TMP_BCDATAREAD(IBC)%BCDATA(6) = ALBEDO(IBC)
    ENDDO
    DEALLOCATE(I2,PERIM_ELEM,ALBEDO,ANGLE,ALIGN,ISPERIM)
    DEALLOCATE(GG%BCDATAREAD)
    GG%BCDATAREAD=>TMP_BCDATAREAD
    GG%NBBCDA=TMP_NBBCDA
    GG%ALBEDO=1.D0
  END SUBROUTINE SALFOLD_1
  !
  SUBROUTINE SALFOLD_2(HSYM,GG)
    !
    !---------------------------------------------------------------------
    !
    !Purpose:
    ...
    !
    ! allocatable arrays
    LOGICAL, ALLOCATABLE, DIMENSION(:) :: ELEM_DUP
    INTEGER, POINTER, DIMENSION(:,:) :: TMP_IPAR
    INTEGER, ALLOCATABLE, DIMENSION(:,:) :: I2
    REAL(PDB), POINTER, DIMENSION(:,:) :: TMP_RPAR
    !
    ...
    !
    NSYM=0
    IF(HSYM=='SR60') THEN ! define rotation center
      NSYM=3
      CENTER_X=XMIN+0.5_PDB*LENGTHX; CENTER_Y=YMIN+0.5_PDB*SQRT(3._PDB)*LENGTHX
      THROT(1)=0.0_PDB; THROT(2)=-PI/3.0_PDB; THROT(3)=PI/3.0_PDB
    ELSE IF(HSYM=='R180') THEN
      NSYM=2
      CENTER_X=XMIN+0.5_PDB*LENGTHX; CENTER_Y=YMIN+0.25_PDB*SQRT(3._PDB)*LENGTHX
      THROT(1)=0.0_PDB; THROT(2)=PI; 
    ELSE IF(HSYM=='R120') THEN
      NSYM=3
      CENTER_X=XMIN+0.5_PDB*LENGTHX/1.5_PDB; CENTER_Y=YMIN+0.5_PDB*SQRT(3._PDB)*LENGTHX/1.5_PDB
      THROT(1)=0.0_PDB; THROT(2)=-2.0_PDB*PI/3.0_PDB; THROT(3)=2.0_PDB*PI/3.0_PDB
    ELSE
      CALL XABORT('SALFOLD_2: invalid type of symmetry axis')
    ENDIF
    XMIN=1.E10_PDB; YMIN=1.E10_PDB; XMAX=-1.E10_PDB; YMAX=-1.E10_PDB;
    !
    ! allocate new surfacic element containers
    ...
    DO ELEM=1,GG%NB_ELEM
      ...
      DO ISYM=1,NSYM
        ...
        IF(TYPE==1) THEN
          XMIN=MIN(XMIN,X1); YMIN=MIN(YMIN,Y1); XMAX=MAX(XMAX,X1); YMAX=MAX(YMAX,Y1);
          X2=X1+GG%RPAR(3,ELEM); Y2=Y1+GG%RPAR(4,ELEM);
          CALL SALROT(CENTER_X,CENTER_Y,THROT(ISYM),X1,Y1,X4,Y4)
          CALL SALROT(CENTER_X,CENTER_Y,THROT(ISYM),X2,Y2,DX4,DY4)
        ELSE IF(TYPE==2) THEN
          CALL SALROT(CENTER_X,CENTER_Y,THROT(ISYM),X1,Y1,X4,Y4)
          THETA1=0._PDB; THETA2=0._PDB;
        ELSE IF(TYPE==3) THEN
          DELPHI=GG%RPAR(5,ELEM)-GG%RPAR(4,ELEM)
          X1B=X1+RAD*COS(GG%RPAR(4,ELEM)); Y1B=Y1+RAD*SIN(GG%RPAR(4,ELEM));
          CALL SALROT(CENTER_X,CENTER_Y,THROT(ISYM),X1,Y1,X4,Y4)
          CALL SALROT(CENTER_X,CENTER_Y,THROT(ISYM),X1B,Y1B,X4B,Y4B)
          IF((ABS(X4B-X4)<EPS*ABS(RAD)).AND.(Y4B-Y4 > 0._PDB)) THEN
            THETA1=PI/2._PDB
          ELSE IF((ABS(X4B-X4)<EPS*ABS(RAD)).AND.(Y4B-Y4 < 0._PDB)) THEN
            THETA1=3._PDB*PI/2._PDB
          ...
          ELSE
            THETA1=ATAN((Y4B-Y4)/(X4B-X4))+PI
          ENDIF
          THETA2=THETA1+DELPHI
          ! put THETA1 within 0 and 2*pi
          IF(THETA1>2._PDB*PI)THEN
            IAUX=INT(THETA1/(2._PDB*PI))
            DELPHI=(2._PDB*PI)*IAUX
            THETA1=THETA1-DELPHI ; THETA2=THETA2-DELPHI
          ELSEIF(THETA1<0._PDB)THEN
            IAUX=INT((-THETA1+1.D-7)/(2._PDB*PI))+1
            DELPHI=(2._PDB*PI)*IAUX
            THETA1=THETA1+DELPHI ; THETA2=THETA2+DELPHI
          ENDIF
        ELSE
        ...
        ELSE IF((TYPE==2).OR.(TYPE==3)) THEN
          TMP_RPAR(3,TMP_NB_ELEM)=GG%RPAR(3,ELEM) ! RADIUS
          TMP_RPAR(4,TMP_NB_ELEM)=THETA1; TMP_RPAR(5,TMP_NB_ELEM)=THETA2; ! ANGLES
          TMP_IPAR(2,TMP_NB_ELEM)=GG%IPAR(2,ELEM); TMP_IPAR(3,TMP_NB_ELEM)=GG%IPAR(3,ELEM)
        ENDIF
        TMP_RPAR(6,TMP_NB_ELEM)=0._PDB
      ENDDO
    ...
    ALLOCATE(ELEM_DUP(TMP_NB_ELEM))
    ELEM_DUP(:TMP_NB_ELEM)=.FALSE.
    DO ELEM=1,TMP_NB_ELEM
      TYPE=TMP_IPAR(1,ELEM)
      IF((TYPE==1).AND.((TMP_IPAR(2,ELEM)==0).OR.(TMP_IPAR(3,ELEM)==0))) THEN
        X4=TMP_RPAR(1,ELEM); Y4=TMP_RPAR(2,ELEM)
        DX4=X4+TMP_RPAR(3,ELEM); DY4=Y4+TMP_RPAR(4,ELEM)
        DO ELEM2=ELEM+1,TMP_NB_ELEM
          IF(TMP_IPAR(1,ELEM2)/=1) CYCLE
          X1=TMP_RPAR(1,ELEM2); Y1=TMP_RPAR(2,ELEM2)
          X2=X1+TMP_RPAR(3,ELEM2); Y2=Y1+TMP_RPAR(4,ELEM2)
          IF(((ABS(X1-X4)<10.0*EPS  .AND. ABS(Y1-Y4)<10.0*EPS  .AND. &
               ABS(X2-DX4)<10.0*EPS .AND. ABS(Y2-DY4)<10.0*EPS)).OR. &
             ((ABS(X1-DX4)<10.0*EPS .AND. ABS(Y1-DY4)<10.0*EPS .AND. &
               ABS(X2-X4)<10.0*EPS  .AND. ABS(Y2-Y4)<10.0*EPS))) THEN
            ELEM_DUP(ELEM2)=.TRUE.
            IF(TMP_IPAR(2,ELEM)==0) TMP_IPAR(2,ELEM)=MAX(TMP_IPAR(2,ELEM2),TMP_IPAR(3,ELEM2))
            IF(TMP_IPAR(3,ELEM)==0) TMP_IPAR(3,ELEM)=MAX(TMP_IPAR(2,ELEM2),TMP_IPAR(3,ELEM2))
            IF(TMP_IPAR(2,ELEM)==TMP_IPAR(3,ELEM)) ELEM_DUP(ELEM)=.TRUE.
            EXIT
          ENDIF
        ENDDO
      ENDIF
    ...
    !
    ELEM=1
    DO WHILE(ELEM<=TMP_NB_ELEM)
      IF(ELEM_DUP(ELEM)) THEN
        TMP_NB_ELEM=TMP_NB_ELEM-1
        DO ELEM2=ELEM,TMP_NB_ELEM
          TMP_IPAR(:,ELEM2)=TMP_IPAR(:,ELEM2+1); TMP_RPAR(:,ELEM2)=TMP_RPAR(:,ELEM2+1)
          ELEM_DUP(ELEM2)=ELEM_DUP(ELEM2+1)
        ENDDO
      ELSE
        ELEM=ELEM+1
      ENDIF
