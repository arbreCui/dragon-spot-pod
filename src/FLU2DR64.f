*DECK FLU2DR64
      SUBROUTINE FLU2DR64(IPRT,IPMACR,IPFLUX,IPSYS,IPTRK,IPFLUP,IPSOU,
...
*   SPOF   : SPOT FLUX SOLUTION DOOR                  EXT ROUTINE
*----
*
      USE GANLIB
      USE SPOMOC_AUDIT, ONLY: SPOMOC_FLU_PATH,SPOMOC_FLU_CONTEXT,
     1 SPOMOC_DOOR_BEGIN
*----
...
*     IUNK_EVAL=1 only when the outer-flux residual was evaluated.
     ...
     1 BFLNOR,DDELN1,DDELD1,PROD,FLXIN
      DOUBLE PRECISION EINN,GINN,FINN,EEXT,ERRDEB1
      TYPE(C_PTR) JPFLUX64
      REAL, ALLOCATABLE, DIMENSION(:) :: GAR32
      REAL, ALLOCATABLE, DIMENSION(:,:) :: FLX32
      LOGICAL LSCAL,LEXAC,REBFLG,LSPOTFS
      REAL ALBEDO(6),FLUXC(NREG),B2(4)
      DOUBLE PRECISION EINR_LAST,EUNK_LAST
*
************************************************************************
*                                                                      *
...
*----
     ...
     1 MATALB
      REAL, ALLOCATABLE, DIMENSION(:) :: DHOM,FXSOR,XSCAT,GAMMA,V,FL,
     1 DFL,LEAK1D
...
*----
     ...
     1 DHOM(NGRP),DIFHET(NMERG,NGRP),XCSOU(NGRP),LEAK1D(NGRP))
      ALLOCATE(GAR32(NUNKNO))
*
      REBFLG=.TRUE.
      IPREB=IPMACR
*
...
*----
      LSCAL=(INSB.EQ.0)
      CALL SPOMOC_FLU_PATH(LSCAL)
      IF(CXDOOR.NE.'SPOT') CALL XABORT('FLU2DR64: SPOT DOOR ONLY.')
      IF((ITYPEC.NE.2).AND.(ITYPEC.NE.3))
     1 CALL XABORT('FLU2DR64: TYPE K ONLY.')
      IF((ILEAK.NE.0).AND.(ILEAK.NE.3))
     1 CALL XABORT('FLU2DR64: ONLY B1 SIGS OR NO LEAKAGE.')
      IF(LREBAL) CALL XABORT('FLU2DR64: USE REBA OFF.')
      IF(NCPTA.NE.0) CALL XABORT('FLU2DR64: USE ACCE N 0.')
      IF(.NOT.LFORW) CALL XABORT('FLU2DR64: FORWARD ONLY.')
*
      CALL KDRCPU(CPU0)
      IF(ILEAK.LT.6) THEN
        INORM=1
...
*----
      ...
      IF(LFORW) THEN
         ...
         ELSE
           ...
           JPFLUX64=LCMLID(IPFLUX,'FLUX64',NGRP)
         ENDIF
      ELSE
         IF(ITYPEC.EQ.1) THEN
      ...
      IF(LFORW) THEN
         CALL LCMGDL(JPFLUX,IG,GAR32)
      ELSE
         CALL LCMGDL(JPFLUX,NGRP-IG+1,GAR32)
      ENDIF
      FLUX(:NUNKNO,IG,2)=DBLE(GAR32(:NUNKNO))
*
...
*----
      ...
      ELSE
         OLDBIL=0.0D0
         ALLOCATE(FLX32(NUNKNO,NGRP))
         FLX32(:NUNKNO,:NGRP)=REAL(FLUX(:NUNKNO,:NGRP,2))
         CALL FLUKEF(IPRT,IPMACR,NGRP,NREG,NUNKNO,NMAT,NIFIS,NANIS,
     1   MATCOD(1),VOL,KEYFLX(1,1,1),XSTRC,XSDIA,XSNUF,XSCHI,NMERG,
     2   IMERG,DIFHET,FLX32,B2,ILEAK,LEAKSW,OLDBIL,AKEFF,AFLNOR)
         DEALLOCATE(FLX32)
         AKEFFO=AKEFF
         AKEEP(2)=AKEFF
      ENDIF
...
*           Todorova heterogeneous leakage model.
     ...
     1      VOL,MATCOD,NMERG,IMERG,KEYFLX(1,1,1),FLX32,B2(4),
     2      IPRT,DIFHET,DHOM)
            DEALLOCATE(FLX32)
            GAMMA(:NGRP)=1.0
         ELSE
*           FUNDAMENTAL MODE CONDITION.
     ...
     1      NMAT,NIFIS,VOL,MATCOD,KEYFLX(1,1,1),FLX32,REFKEF,
     2      IPRT,DIFHET(1,1),GAMMA,AKEFF,INORM,B2,LEAK1D)
            DEALLOCATE(FLX32)
         ENDIF
...
*----
  ...
  150 CONTINUE
      ...
      IF((.NOT.LSCAL).AND.(ILEAK.EQ.0)) THEN
         IDIR=0
         CALL SPOMOC_FLU_CONTEXT(IT,JT)
         CALL SPOMOC_DOOR_BEGIN()
         CALL FLUSP64(JPSYS,NPSYS,IPTRK,IPRT,NGRP,NMAT,NREG,NUNKNO,
     1   MATCOD,VOL,KEYFLX,TITLE,ILEAK,B2,NMERG,IMERG,DIFHET,
     2   FLUX(1,1,2),FLUX(1,1,8),FLUX(1,1,7))
...
*
         ...
         IF(ILEAK.EQ.0) THEN
           IDIR=0
           CALL FLUSP64(JPSYS,NPSYS,IPTRK,IPRT,NGRP,NMAT,NREG,
     1     NUNKNO,MATCOD,VOL,KEYFLX,TITLE,ILEAK,B2,NMERG,IMERG,DIFHET,
     2     FLUX(1,1,2),FLUX(1,1,8),FLUX(1,1,7))
         ELSE
           CALL FLUSP64(JPSYS,NPSYS,IPTRK,IPRT,NGRP,NMAT,NREG,
     1     NUNKNO,MATCOD,VOL,KEYFLX,TITLE,ILEAK,B2,NMERG,IMERG,DIFHET,
...
*         NO B-N LEAKAGE CALCULATION REQUIRED
          ...
          ALLOCATE(FLX32(NUNKNO,NGRP))
          FLX32(:NUNKNO,:NGRP)=REAL(FLUX(:NUNKNO,:NGRP,3))
          CALL FLUKEF(IPRT,IPMACR,NGRP,NREG,NUNKNO,NMAT,NIFIS,NANIS,
     1    MATCOD(1),VOL,KEYFLX(1,1,1),XSTRC,XSDIA,XSNUF,XSCHI,NMERG,
     2    IMERG,DIFHET,FLX32,B2,ILEAK,LEAKSW,OLDBIL,AKEFF,AFLNOR)
          DEALLOCATE(FLX32)
...
*           Todorova heterogeneous leakage model.
     ...
     1      NMAT,VOL,MATCOD,NMERG,IMERG,KEYFLX(1,1,1),FLX32,B2(4),
     2      IPRT,DIFHET,DHOM)
            DEALLOCATE(FLX32)
            IF(.NOT.LEAKSW) THEN
              LEAK1D(:NGRP)=0.0
              ALLOCATE(FLX32(NUNKNO,NGRP))
              FLX32(:NUNKNO,:NGRP)=REAL(FLUX(:NUNKNO,:NGRP,3))
              CALL B1HOM(IPMACR,LEAKSW,NUNKNO,'LKRD',HTYPE(ITYPEC),
     1        NGRP,NREG,NMAT,NIFIS,VOL,MATCOD,KEYFLX(1,1,1),FLX32,
     2        REFKEF,IPRT,DHOM(1),GAMMA,AKEFF,INORM,B2,LEAK1D)
              DEALLOCATE(FLX32)
              GO TO 350
            ENDIF
         ENDIF
...
*           FUNDAMENTAL MODE CONDITION.
     ...
     1      NREG,NMAT,NIFIS,VOL,MATCOD,KEYFLX(1,1,1),FLX32,
...
*              COMPUTE THE DIRECTIONNAL BUCKLING COMPONENTS FOR TIBERE.
               ...
               IF(IHETL.GT.0) THEN
                  ALLOCATE(FLX32(NUNKNO,NGRP))
                  FLX32(:NUNKNO,:NGRP)=REAL(FLUX(:NUNKNO,:NGRP,3))
                  CALL FLUBLN(IPMACR,IPRT,NGRP,NMAT,NREG,NUNKNO,
     1            NIFIS,MATCOD,VOL,KEYFLX(1,1,1),FLX32,IHETL,REFKEF,B2)
                  DEALLOCATE(FLX32)
               ENDIF
             ENDIF
          ENDIF
...
*       FLUX NORMALIZATION TO KEFF.
        IF(ITYPEC.LT.5) THEN
          FLUX(:NUNKNO,:NGRP,3)=FLUX(:NUNKNO,:NGRP,3)*(AKEFF/FISOUR)
          FLUX(:NUNKNO,:NGRP,4)=FLUX(:NUNKNO,:NGRP,4)*(AKEFF/FISOUR)
        ENDIF
      ENDIF
*----
*  PRINT TIME TAKEN
...
*----
      DO 510 IG=1,NGRP
      GAR32(:NUNKNO)=REAL(FLUX(:NUNKNO,IG,3))
      CALL LCMPDL(JPFLUX,IG,NUNKNO,2,GAR32)
      CALL LCMPDL(JPFLUX64,IG,NUNKNO,4,FLUX(1,IG,3))
      GAR32(:NUNKNO)=REAL(FLUX(:NUNKNO,IG,4))
      CALL LCMPDL(JPSOUR,IG,NUNKNO,2,GAR32)
  510 CONTINUE
      IF(C_ASSOCIATED(IPSOU)) THEN
      ...
      IF(ITYPEC.GE.2) THEN
         ...
         CALL LCMPUT(IPFLUX,'SPOT-KEFF64',1,4,AKEFF)
...
*----
      DEALLOCATE(LEAK1D,XCSOU,DIFHET,DHOM,GAMMA,XSCAT,FLUX,GAR32)
...
*
 ...
 1190 FORMAT(' FLU2DR-TERM INNER-TERMINAL ITERF=',I6,' MAXINR=',I6,
     ...
     2 ' STATE=',I1,' NGRP=',I6)
      END
*DECK FLUSP64
      SUBROUTINE FLUSP64(JPSYS,NPSYS,IPTRK,IMPX,NGRP,NMAT,NREG,NUN,
     1 MAT,VOL,KEYFLX,TITR,ILEAK,B2,NMERG,IMERG,DIFHET,FLUOLD,SUN,FUN)
*
*  REAL64 SPOT-door dispatch for FLU2DR64: applies the SIGS (ILEAK=3)
*  leakage source correction in double precision and calls SPOF64.
*
      USE GANLIB
      TYPE(C_PTR) JPSYS,IPTRK
      INTEGER NPSYS(NGRP),IMPX,NGRP,NMAT,NREG,NUN,MAT(NREG),
     1 KEYFLX(NREG),ILEAK,NMERG,IMERG(NMAT)
      REAL VOL(NREG),B2(4),DIFHET(NMERG,NGRP)
      CHARACTER TITR*72
      DOUBLE PRECISION FLUOLD(NUN,NGRP),SUN(NUN,NGRP),FUN(NUN,NGRP)
      TYPE(C_PTR), ALLOCATABLE, DIMENSION(:) :: KPSYS
      INTEGER, ALLOCATABLE, DIMENSION(:) :: NGIND
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:) :: F2,S2
      INTEGER IG,JJ,NGEFF,IR,IND,IBM,INM
      IF((ILEAK.NE.0).AND.(ILEAK.NE.3))
     1 CALL XABORT('FLUSP64: UNSUPPORTED LEAKAGE MODEL.')
      NGEFF=0
      DO IG=1,NGRP
        IF(NPSYS(IG).NE.0) NGEFF=NGEFF+1
      ENDDO
      IF(NGEFF.EQ.0) RETURN
      ALLOCATE(KPSYS(NGEFF),NGIND(NGEFF),F2(NUN,NGEFF),S2(NUN,NGEFF))
      JJ=0
      DO IG=1,NGRP
        IF(NPSYS(IG).NE.0) THEN
          JJ=JJ+1
          NGIND(JJ)=IG
          KPSYS(JJ)=LCMGIL(JPSYS,NPSYS(IG))
          S2(:NUN,JJ)=SUN(:NUN,IG)
          F2(:NUN,JJ)=FUN(:NUN,IG)
          IF((ILEAK.EQ.3).AND.(B2(4).NE.0.0)) THEN
            DO IR=1,NREG
              IND=KEYFLX(IR)
              ...
              INM=IMERG(IBM)
              IF(INM.EQ.0) GO TO 10
              S2(IND,JJ)=S2(IND,JJ)-DBLE(DIFHET(INM,IG))*DBLE(B2(4))*
     1        FLUOLD(IND,IG)
   10         CONTINUE
            ENDDO
          ENDIF
        ENDIF
      ENDDO
      CALL SPOF64(KPSYS,IPTRK,IMPX,NGEFF,NGIND,NREG,NMAT,NUN,MAT,
     1 KEYFLX,F2,S2,TITR)
      DO JJ=1,NGEFF
        FUN(:NUN,NGIND(JJ))=F2(:NUN,JJ)
      ENDDO
      DEALLOCATE(S2,F2,NGIND,KPSYS)
      RETURN
      END
