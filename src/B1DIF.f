*DECK B1DIF
      SUBROUTINE B1DIF(OPTION,TYPE,NGRO,ST,SFNU,XHI,LEAK1D,IJJ0,IJJ1,
     1 NJJ0,NJJ1,SCAT0,SCAT1,REFKEF,LFISSI,IMPX,DHOM,GAMMA,B2,ALAM1,
     2 CAET,A2,PHI)
*
*-----------------------------------------------------------------------
*
...
*----
      ...
      REAL ST(NGRO),SFNU(NGRO),XHI(NGRO),LEAK1D(NGRO),SCAT0(*),SCAT1(*),
     > DHOM(NGRO),GAMMA(NGRO)
...
*----
   ...
   40 CONTINUE
      ...
      IGAR=IGAR+1
      IF(J.EQ.I) THEN
        ASTOC(I,I)=ASTOC(I,I)-(1.0D0-B2*BETA)*(SCAT0(IGAR)-
     >  LEAK1D(I))
        BSTOC(I,I)=SCAT0(IGAR)-LEAK1D(I)
      ELSE
...
*----
  ...
  220    CONTINUE
      ...
      IGAR=IGAR+1
      IF(J.EQ.I) THEN
        ASTOC(I,I)=ASTOC(I,I)-SCAT0(IGAR)+LEAK1D(I)
      ELSE
