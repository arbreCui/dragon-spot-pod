...
*----
      ...
      TYPE(C_PTR) IPFLUX,IPTRK,IPMACR,IPSYS,IPFLUP,IPSOU,IPSNAP
...
*----
     ...
     1    5H (1).)') TRIM(HENTRY(I1)),TRIM(HSIGN)
          CALL XABORT(HSMG)
        ENDIF
      ELSE IF(LTABLE.AND.(JENTRY(I1).EQ.1)) THEN
        CALL LCMGTC(KENTRY(I1),'SIGNATURE',12,HSIGN)
        IF((HSIGN.EQ.'L_ARCHIVE').AND.(.NOT.C_ASSOCIATED(IPSNAP))) THEN
          IPSNAP=KENTRY(I1)
        ELSE
          WRITE(HSMG,'(20HFLU: UNKNOWN OBJECT ,A,14H OF SIGNATURE ,A,
     1    5H (2).)') TRIM(HENTRY(I1)),TRIM(HSIGN)
          CALL XABORT(HSMG)
        ENDIF
      ELSE
...
*       FIXED SOURCE EIGENVALUE PROBLEM
     ...
     4  MATCOD,KEYFLX,VOL,REFKEF,NMERG,IMERG)
      ENDIF
*----
*  STORE AXIAL LEAKAGE COEFFICIENTS FOR THE SPOT METHOD
*----
      IF(CXDOOR.EQ.'SPOT') THEN
         IF(ISPOT.NE.-1) CALL XABORT('FLU: MISSING SPOT INFORMATION.')
         CALL SPODB2(IPFLUX,IPTRK,IPSNAP,NUN,NGRP,NREG,MATCOD,KEYFLX,
     1   VOL,IPICK,IPRINT)
      ENDIF
*----
