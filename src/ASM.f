...
*----
     ...
     1    //'MED '//TEXT12//' IS NOT IN REAL-ONLY MODE.')
        ...
        ELSE IF(IENTRY(4).LE.2) THEN
          ! read snapshot information for the spot method
          IF(JENTRY(4).NE.2) CALL XABORT('ASM: SNAPSHOT ARCHIVE '//
...
*----
   ...
   40 IF(INDIC.NE.3) CALL XABORT('ASM: CHARACTER DATA EXPECTED(2).')
     ...
     1   //'HOT OBJECT(1).')
         CALL REDGET(INDIC,ISPOT,FLOTT,TEXT4,DFLOTT)
         IF(INDIC.NE.1) CALL XABORT('ASM: INTEGER DATA EXPECTED(2).')
         IF((ISPOT.LE.0).OR.(ISPOT.GT.NSNAP)) THEN
            WRITE(HSMG,'(21HASM: SNAPSHOT INDEX (,I3,14H) MUST BE 0<IS,
     1      5HPOT<=,I3,1H.)') ISPOT,NSNAP
            CALL XABORT(HSMG)
         ENDIF
      ELSE IF(TEXT4.EQ.'LK2D') THEN
         IF(.NOT.C_ASSOCIATED(IPSNAP)) CALL XABORT('ASM: MISSING SNAPS'
     1   //'HOT OBJECT(2).')
         ISPOT=-1
      ELSE IF(TEXT4.EQ.'POD') THEN
*        Proper Orthogonal Decomposition compression of LEAK2D.
*        Syntax: POD <eps_pod> <rank_max>
         CALL REDGET(INDIC,NITMA,EPS_POD,TEXT4,DFLOTT)
         IF(INDIC.NE.2) CALL XABORT('ASM: POD EPS REAL EXPECTED.')
         CALL REDGET(INDIC,RANK_POD,FLOTT,TEXT4,DFLOTT)
         IF(INDIC.NE.1) CALL XABORT('ASM: POD RANK INTEGER EXPECTED.')
...
*  SPOT ASSEMBLY
*----
   60 IF(ISPOT.EQ.-1) THEN
        CALL SPOASM(IPSYS,IPMACR,IPTRK,IPSNAP,IMPX,EPS_POD,RANK_POD)
        GO TO 70
      ENDIF
*----
...
*----
     ...
     2 ISPOT,TITRE,ITPIJ,LNORM.OR.LALBS,IPHASE,ISTRM,KNORM,NALBP)
*
...
*
 ...
 6040 FORMAT(/8H OPTIONS/8H -------/
     ...
     6 7H ISPOT ,I8,45H   (-1/0/>0: SPOT LEAKAGE 2D-LEAKAGE/NONE/1D))
      END
